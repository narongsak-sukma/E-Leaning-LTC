-- ═══ 0048 — D77 หน้าต่าง A (Wave G P2): cardinality re-check หลัง selection ═══
-- ทำไม: two-session probe ของ IT wave-g-qb-bank-detail จับจริงว่า non-mix branch
-- ของ start_attempt นับ pool (:183) กับหยิบจริง (:197) เป็นคนละ statement —
-- Read Committed อ่าน snapshot ใหม่ทุก statement ทำให้ retire ที่ commit แทรก
-- กลางคัน (ผ่านการนับ → ก่อนการหยิบ) เห็นตอนหยิบแต่ไม่เห็นตอนนับ → v_qids
-- สั้นกว่า rules.question_count → attempt ถูกสร้างด้วย question_count ที่ไม่ครบ
-- (invariant snapshot = attempt.question_count = rules.question_count แตก)
-- แก้: เพิ่ม re-check หลัง selection ด้วยแบบแผนเดียวกับ mix branch ที่มีอยู่แล้ว
-- (0022:174-176) — หยิบไม่ครบ = คลังข้อไม่พอ ERR-ASM-003 · TX กลิ้งทั้งก้อน
-- ไม่มี attempt ค้าง · mix branch / snapshot CTE (หน้าต่าง B ยอมรับข้อที่เลือกไว้
-- ตาม r4) คงเดิมทุกไบต์
-- โครงสร้าง: copy start_attempt ทั้งฟังก์ชันจาก 0022 (แบบแผน 0019/0020/0022
-- ที่แทนฟังก์ชันด้วยสำเนาเต็ม) แก้จุดเดียว: แทรก if cardinality(v_qids) <
-- v_rules.question_count หลัง select ... limit v_rules.question_count) t;
-- ของ non-mix branch · guards ทุกตัวอื่นคงเดิมทุกไบต์

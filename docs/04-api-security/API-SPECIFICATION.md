...
| POST | /admin/users | สร้างบัญชีเจ้าหน้าที่ (บังคับ MFA ตั้งแต่วันแรก — GoTrue admin createUser + profiles + role เริ่มต้น ผ่าน service lib จุดเดียว · audit `USER_CREATE`) | super_admin | 201 | RBAC-001, VAL-001 |
| PATCH | /admin/users/{id} | แก้ข้อมูล/ปิด-เปิดใช้งานบัญชี — **ปิดใช้งานบังคับจริงด้วย GoTrue ban/unban** (ban = login ไม่ได้ทันที) + `reason` บังคับเมื่อ disable · audit `USER_DISABLE`/`USER_UPDATE` · **ห้ามแตะบัญชี super_admin ตัวอื่น** โดยผู้ไม่ใช่ super_admin (BFF guard 403) | staff:registrar (ข้อมูลสมาชิก), super_admin (ทุกกรณี) | 200 | RBAC-001, VAL-001 |
| GET | /admin/roles | รายการบทบาท + คำอธิบายสิทธิ์ | staff:viewer, super_admin | 200 | RBAC-001 |
| POST | /admin/users/{id}/roles | มอบบทบาท — atomic RPC `admin_grant_role` (0035 r2 · mutation+audit TX เดียว · aal2 ตรวจใน RPC): super_admin มอบ `lawyer`/`instructor`/`staff:*` ได้ · **staff:registrar มอบได้เฉพาะ `lawyer` หลังยืนยันใบอนุญาต** (ขั้น BFF resource-scope → `ERR-RBAC-001\|role_scope` · ขั้น RPC เช่นกัน) · **`super_admin` ห้ามผ่าน endpoint นี้** (bootstrap เท่านั้น — fail-closed → `ERR-VAL-001\|role_not_grantable`) · ถืออยู่แล้ว = idempotent `granted:false` (201) · audit `ROLE_GRANT` เฉพาะเมื่อเกิด mutation | super_admin; staff:registrar (เฉพาะ `lawyer` หลังยืนยันใบอนุญาต) | 201 | RBAC-001, VAL-001 |
| DELETE | /admin/users/{id}/roles | ถอนบทบาท — atomic RPC `admin_revoke_role` (0035 r2 · เงื่อนไขสิทธิ์เดียวกับการมอบ) · **ถอน role สุดท้ายของบัญชี = 400** `ERR-VAL-001\|last_role` (errcode 22023 — บัญชีต้องมี role ≥1 เสมอ · TX rollback แถวเดิมคงใช้งาน) · ถอน `super_admin`/ค่านอก enum = 400 `ERR-VAL-001\|role_not_manageable` · `citizen` ถอนได้ตามปกติ (เงื่อนไขเดียวที่กันคือ last_role) · audit `ROLE_REVOKE` | เช่นเดียวกับด้านบน | 204 | RBAC-001, VAL-001 |

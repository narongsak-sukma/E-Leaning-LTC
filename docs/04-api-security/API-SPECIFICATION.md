...
| POST | /admin/certificates/bulk | ออกเป็นชุด — job ตาราง `cert_bulk_jobs` (DD §3.7) · RPC batch 200 · TX เดียวต่อใบ + audit `CERT_ISSUE` actor ครบต่อใบ (context เพิ่ม `mode:'bulk'` — D55-5) · คิว eligible กรองก่อนตัดหน้า (filter-before-cut) — **Wave E Phase 2 (E-4): runner RPC `admin_cert_bulk_issue_run` พร้อมใน migration 0026 (DCR-8) หน้า BFF กำลังตาม** | staff:registrar, super_admin | 202 (job) | RBAC-001, VAL-001 |
| GET | /admin/certificates/bulk/{jobId} | สถานะ job ออกใบเป็นชุด (status/total/issued/failed/last_error ตัดทอน) — ผู้สร้าง job + super_admin — **Wave E Phase 2 (E-4 · DCR-8)** | staff:registrar, super_admin | 200 | RBAC-001, NF-001 |

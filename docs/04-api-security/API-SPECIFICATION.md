...
| PATCH | /me | แก้โปรไฟล์ — **เขต `display_name`/`phone`/`preferred_locale` เท่านั้น** (guard 0010 · D20-M2): `first_name`/`last_name` เป็นข้อมูลนิติบุคคล แก้ผ่านเจ้าหน้าที่ (DCR-11/DD) — ฟิลด์ sensitive (อีเมล) แยก endpoint (Wave F) | ทุกบทบาทที่ login แล้ว | 200 | VAL-001 |
...
| Method | Path | คำอธิบาย | บทบาท | Success | Errors |
| --- | --- | --- | --- | --- | --- |
| GET | /admin/users | ค้นหา/กรองผู้ใช้ (display_name/อีเมล/สถานะ — keyset ผ่าน RPC `admin_list_users` · audit `PII_ACCESS` **fail-closed**: audit RPC ล้มสองครั้ง = 503 ไม่เปิดเผยข้อมูล — แบบแผน 1.1.2 B7) | staff:viewer, staff:registrar, super_admin | 200 + pagination | RBAC-001 |
...
| DELETE | /admin/users/{id}/roles | ถอนบทบาท — **body บังคับ `{role, reason 10-500}`** (DELETE พร้อม body ตามที่ RPC กำหนด — reason เป็นของ audit `ROLE_REVOKE`; ไม่มี body/ผิดรูป = 400 ก่อนแตะ RPC) · atomic RPC `admin_revoke_role` (0035 r2 · เงื่อนไขสิทธิ์เดียวกับการมอบ) · **ถอน role สุดท้ายของบัญชี = 400** `ERR-VAL-001\|last_role` (errcode 22023 — บัญชีต้องมี role ≥1 เสมอ · TX rollback แถวเดิมคงใช้งาน) · ถอน `super_admin`/ค่านอก enum = 400 `ERR-VAL-001\|role_not_manageable` · `citizen` ถอนได้ตามปกติ (เงื่อนไขเดียวที่กันคือ last_role) · audit `ROLE_REVOKE` | เช่นเดียวกับด้านบน | 204 | RBAC-001, VAL-001 |

...
| POST | /admin/credit-rules | สร้างกฎใหม่ (config-driven, อ้าง Q1 · status เริ่มต้น draft) + audit `CREDIT_RULE_CREATE` (BFF service_role RPC — allowlist 0032 best-effort ตามแบบแผน ADMIN_EXPORT) | staff:registrar, super_admin | 201 | CRD-001, VAL-001 |
| PATCH | /admin/credit-rules/{id} | แก้กฎ — **lifecycle เท่านั้น** (status draft→active→retired) — trigger `guard_rule_semantics` (0010) บังคับ: แก้คอลัมน์ semantic ของแถว active/ถูกอ้าง = ยกเว้นเฉพาะสร้างเวอร์ชันใหม่ + audit `CREDIT_RULE_UPDATE` (BFF service_role RPC — 0032) | staff:registrar, super_admin | 200 | CRD-001 |
| GET | /admin/credits/{userId} | รายการ ledger ของผู้ใช้รายคน (keyset) (เข้าถึงข้อมูลรายบุคคล → audit `PII_ACCESS` best-effort) | staff:registrar, staff:viewer, super_admin | 200 | RBAC-001 |

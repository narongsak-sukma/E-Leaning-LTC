...
| POST | /admin/users/{id}/roles | มอบบทบาท — atomic RPC `admin_grant_role` (0035 · mutation+audit TX เดียว · aal2 ตรวจใน RPC): super_admin มอบ `lawyer`/`instructor`/`staff:*` ได้ · **staff:registrar มอบได้เฉพาะ `lawyer` หลังยืนยันใบอนุญาต** (BFF resource-scope 403 ก่อนถึง RPC) · **`super_admin` ห้ามผ่าน endpoint นี้** (bootstrap เท่านั้น — fail-closed) · audit `ROLE_GRANT` | super_admin; staff:registrar (เฉพาะ `lawyer` หลังยืนยันใบอนุญาต) | 201 | RBAC-001, VAL-001 |
| DELETE | /admin/users/{id}/roles | ถอนบทบาท — atomic RPC `admin_revoke_role` (เงื่อนไขสิทธิ์เดียวกับการมอบ) · **ถอน role สุดท้ายของบัญชี = 422** (บัญชีต้องมี role ≥1 เสมอ) · audit `ROLE_REVOKE` | เช่นเดียวกับด้านบน | 204 | RBAC-001, VAL-001 |
...
| GET | /admin/dashboard | ตัวเลขสรุปหน้าแรก admin — RPC `admin_dashboard_stats(p_from, p_to)` (0036 · aggregate สด lag ≤15 นาที — ไม่ต้อง materialized view): ผู้ใช้ใหม่/รวม · การลงทะเบียน · ผลสอบ (ผ่าน/เรท) · credit ที่ออก · กรองช่วงวันที่ (แสดงตามสิทธิ์ของบทบาทที่ถือ — ใช้สิทธิ์ report:view) | staff:viewer, staff:content, staff:exam, staff:registrar, super_admin | 200 | RBAC-001 |

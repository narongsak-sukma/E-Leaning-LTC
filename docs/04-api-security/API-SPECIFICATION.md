...
| POST | /admin/credit-rules | สร้างกฎใหม่ (config-driven, อ้าง Q1) | staff:registrar, super_admin | 201 | CRD-001, VAL-001 |
| PATCH | /admin/credit-rules/{id} | แก้กฎ — **lifecycle เท่านั้น** (status draft↔active/retired · effective_to · note) — trigger `guard_rule_semantics` (0010) บังคับ: แก้คอลัมน์ semantic ของแถว active/ถูกอ้าง = ยกเว้นเฉพาะสร้างเวอร์ชันใหม่ | staff:registrar, super_admin | 200 | CRD-001 |
| GET | /admin/users/{id}/credits | ยอด credit ของผู้ใช้รายคนตามรอบ + รายการ ledger (keyset) (เข้าถึง PII → audit `PII_ACCESS`) | staff:registrar, staff:viewer, super_admin | 200 | RBAC-001 |
| POST | /admin/credit-adjustments | ปรับ credit มือ (+/−) **ต้องมี reason 10-500 ตัวอักษร + สร้างรายการ adjustment ใหม่ใน ledger (append-only) + audit `CREDIT_ADJUST`** · แจ้งเจ้าของ = Phase 4 (NTF) | staff:registrar, super_admin | 201 | CRD-002, RBAC-001 |

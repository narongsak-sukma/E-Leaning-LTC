...
| AUTH_MFA_BACKUPS_REGENERATED | ทุกบทบาท (MFA แล้ว) — ผู้บันทึกจริง = RPC `mfa_backup_codes_replace` (0042 atomic · ตรวจ TOTP verified + hash 8-12 ค่า + distinct ในตัว · ชุดเก่า invalid ทันที) | count, replaced_previous | NOTICE | — (ไม่เก็บโค้ด/hash) |
| AUTH_MFA_BACKUP_CODE_USED | เจ้าของบัญชี (ขั้นสองของ login) — ผู้บันทึกจริง = RPC `mfa_backup_codes_consume` (0042 · single-use ผ่านเงื่อนไข `used_at is null` ใน UPDATE เดียว — สองยิงพร้อมกันผ่านรายการเดียว) | remaining (โค้ดเหลือ) | NOTICE | — (ไม่เก็บโค้ด/hash) |
| AUTH_MFA_BACKUPS_INVALIDATED | เจ้าของบัญชี (ปิด MFA/unenroll factor สุดท้าย) — ผู้บันทึกจริง = RPC `mfa_backup_codes_invalidate` (0042 · เกิดเฉพาะมีแถวถูกลบจริง) | removed | NOTICE | — (ไม่เก็บโค้ด/hash) |
...
| USER_EMAIL_CHANGE_REQUEST | เจ้าของบัญชี — ผู้บันทึกจริง = RPC `my_audit_email_change_request` (0044 · **0045 (gate r1 F3): BFF เรียก RPC ก่อน GoTrue `updateUser({email})` เสมอ** — guard aal2/role ที่ DB ตัดสินก่อนเกิด mutation · updateUser ล้มหลัง audit = REQUEST ยังตรงความหมาย "มีคำขอ" · กรณี email_taken ตอบ masked-as-success แต่**ยังถูก audit** (หลักฐานภายใน sha256 ล้วน) · บัญชีบทบาทบังคับ MFA ต้อง aal2 — SoD เดียวกับการลบบัญชี) | new_email_sha256 (hash เท่านั้น — ห้ามอีเมลจริง) | NOTICE | การเปลี่ยนข้อมูลติดต่อของเจ้าของข้อมูล |
| USER_EMAIL_CHANGE_CONFIRMED | เจ้าของบัญชี — ผู้บันทึกจริง = trigger `audit_email_change_confirmed` บน auth.users (0044 · จุดเดียวที่รู้ว่า GoTrue ผูกอีเมลใหม่จริงหลัง double opt-in · **0045 (gate r1 F7): fail-closed — audit ล้ม = การ update อีเมลล้ม** (เหตุการณ์เปลี่ยน identity ห้ามหายเงียบ · trade-off ตัดสินโดย gate: availability ของ flow หายาก < ความสมบูรณ์ของ audit)) | old_email_sha256, new_email_sha256 | NOTICE | การเปลี่ยนตัวตนการติดต่อ (identifier ของบัญชี) |
...
| QB_QUESTION_STATUS | staff:exam, super_admin — ผู้บันทึกจริง = RPC `admin_set_question_status` (0047 · D75 · เขียนทางเดียวหลัง revoke UPDATE ตรงบน questions): TX เดียว = UPDATE status + version+1 + audit — audit ล้ม = rollback ทั้ง TX · `p_request_id` จาก header `x-request-id` ลงช่องจริง (Wave G P2) | question_id, from, to, version | NOTICE | — |
...
| PURGE_EXECUTED | ระบบ (cron `ltc-purge-retention` 04:23 ใต้บทบาท `purge_role` — actor NULL) — ผู้บันทึกจริง = RPC `purge_expired_retention(false)` (0040 · เฉพาะรันจริง · dry-run ไม่เขียน audit) | job, dry_run=false, purged (จำนวนต่อตาราง — ไม่มี PII) | NOTICE | การทำลายข้อมูลตามกำหนดเก็บ (DD §4.6 — หลักฐานการ purge ตามนโยบาย retention) |

รวม **63 event types** (นับจากตาราง §2.1–2.6 — เพิ่ม AUTH_PASSWORD_CHANGE, QB_QUESTION_DELETE, CERT_REISSUE ตาม A6 review B-14; AUTH_MFA_BACKUPS_REGENERATED ตาม D11-12; EXAM_SESSION_TAKEOVER ตาม D12-21; CERT_PDF_ATTACH ตาม D38; CREDIT_REVERSAL ตาม DCR-9/D68; 4 events ตาม DCR-11 [1.0.4 — ตัวเลขเดิมในบรรทัดนี้ค้าง 53 ไม่ได้ปรับตาม 1.0.4 จนถึง 1.0.5]; Wave F 0040/0042/0044 เพิ่ม PURGE_EXECUTED, AUTH_MFA_BACKUP_CODE_USED, AUTH_MFA_BACKUPS_INVALIDATED, USER_EMAIL_CHANGE_REQUEST, USER_EMAIL_CHANGE_CONFIRMED; QB_QUESTION_STATUS ตาม Wave G P2 0047/D75)

...
| USER_EMAIL_CHANGE_REQUEST | เจ้าของบัญชี — ผู้บันทึกจริง = RPC `my_audit_email_change_request` (0044 · BFF เรียกหลัง GoTrue `updateUser({email})` สำเร็จ · บัญชีบทบาทบังคับ MFA ต้อง aal2 — SoD เดียวกับการลบบัญชี) | new_email_sha256 (hash เท่านั้น — ห้ามอีเมลจริง) | NOTICE | การเปลี่ยนข้อมูลติดต่อของเจ้าของข้อมูล |
| USER_EMAIL_CHANGE_CONFIRMED | เจ้าของบัญชี — ผู้บันทึกจริง = trigger `audit_email_change_confirmed` บน auth.users (0044 · จุดเดียวที่รู้ว่า GoTrue ผูกอีเมลใหม่จริงหลัง double opt-in · audit ล้ม = RAISE WARNING ไม่ทำให้การยืนยันพัง) | old_email_sha256, new_email_sha256 | NOTICE | การเปลี่ยนตัวตนการติดต่อ (identifier ของบัญชี) |

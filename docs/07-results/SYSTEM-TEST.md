...
- **Coverage ตาม RTM must-have — นับตรงตามแถวของ §3 ทุกแถว** (แก้ตาม codex gate p4-r2 M5: ตัวเลขเดิม "ผ่าน 74 · บางส่วน 5 · 79/83" ไม่ตรง census จริง): **ผ่าน 71 · ผ่านบางส่วน 7** → มีหลักฐานพิสูจน์จริง **78/83** (เกณฑ์: นับเฉพาะแถวที่มีหลักฐานจาก battery/VA-PENTEST จริง ทั้ง "ผ่าน" และ "ผ่านบางส่วน") · อีก **1 แถวปิดตามนโยบาย** (AUTH-003 OTP มือถือ — ปิดตาม D-f-8 ไม่นับเป็น coverage) · **4 รายการไม่มีใน as-built v1** (AUTH-004/005/010 · ASM-012(S)) จดเป็นหนี้ทะเบียนพร้อมเหตุผลที่ §5 — LRN-009 เคยถูกจัด "ไม่มี" ใน 1.0.0/1.0.1 โดยผิด แก้เป็น **ผ่านบางส่วน** ตามหลักฐานโค้ดจริง (§3 — แก้ตาม codex gate p4-r1 M7) — **ไม่มีแถวใดถูกอ้างว่า "ผ่าน" โดยไม่มีหลักฐาน** · แถวบางส่วนทั้ง 7: AUTH-009 · AUTH-011 · CAT-003 · LRN-009 · ASM-011 · CRB-008 · AUD-005
...
หมายเหตุ: ทั้ง 4 รายการเป็น **ความจริงของ as-built v1 ที่ RTM วางแผนไว้กว้างกว่า** — บันทึกเปิด ไม่ปิดบัง และไม่นับเข้า coverage ที่อ้างผ่านของ §3 · **แก้ไขตาม codex gate p4-r1 M7**: LRN-009 เคยอยู่ในตารางนี้ (1.0.0/1.0.1 จัด "ไม่มีฟิลด์/endpoint ตำแหน่งสื่อ") — การตรวจซ้ำพบว่า resume มีจริงในโค้ด (การ์ดเรียนต่อ + ตัวเล่น seek อัตโนมัติจาก watch % — หลักฐานที่ §3) จึงย้ายออกจากหนี้ไปเป็น **ผ่านบางส่วน** · สิ่งที่ยังเป็นข้อจำกัดจริงของ LRN-009 คือความละเอียดของตำแหน่ง (ประมาณจาก % ไม่ใช่วินาทีเป๊ะ) — ถ้าต้องการปิดถึงระดับนั้น ให้ใช้ `video_max_position_sec` ที่บันทึกอยู่แล้วมา seed ตัวเล่นแทน (งานอนาคต ไม่ใช่หนี้ "ไม่มี")

---

## 6. คำวินิจฉัยชุดทดสอบ (สรุปปิด Phase 2)

- ชุดทดสอบที่รันจริงครอบครบทุกฟังก์ชันหลักที่ส่งมอบใน v1 (78/83 FR มีหลักฐาน = ผ่าน 71 + ผ่านบางส่วน 7 ตาม census ของ §3 · อีก 1 ปิดตามนโยบาย AUTH-003 · 4 รายการไม่มีใน as-built จดเปิด · LRN-009 แก้เป็นผ่านบางส่วนตาม gate p4-r1 M7 — ตัวเลขปรับตาม gate p4-r2 M5) และ NFR กลุ่ม security ทั้งหมดมีหลักฐานระดับโค้ด+ชุดทดสอบ
- ความเสี่ยงที่เหลืออยู่รูปธรรม: (1) กลุ่ม PERF รอวัดจริงบน prod (2) การโจมตีเชิงมนุษย์/business logic รอ external pentest (3) หนี้ 4 รายการของ §5.3

...
- **Coverage ตาม RTM must-have — นับตรงตามแถวของ §3 ทุกแถว** (แก้ตาม codex gate p4-r2 M5: ตัวเลขเดิม "ผ่าน 74 · บางส่วน 5 · 79/83" ไม่ตรง census จริง): **ผ่าน 74 · ผ่านบางส่วน 7** → มีหลักฐานพิสูจน์จริง **81/83** (เกณฑ์: นับเฉพาะแถวที่มีหลักฐานจาก battery/VA-PENTEST จริง ทั้ง "ผ่าน" และ "ผ่านบางส่วน") · อีก **1 แถวปิดตามนโยบาย** (AUTH-003 OTP มือถือ — ปิดตาม D-f-8 ไม่นับเป็น coverage) · **1 รายการไม่มีใน as-built v1** (ASM-012(S) เท่านั้น — จดหนี้ §5) · AUTH-004/005/010 เดิมเคยเป็นหนี้ส่วนนี้ **ปิดแล้วโดย Wave G P1 `[#92]`** (merge `fc790c8` — ผ่าน codex gate 4 รอบ r1 FAIL(1B/4M/3m) → r2/r3 FAIL → **r4 PASS 0B/0M/2m ปิด MINOR ครบ**) — LRN-009 เคยถูกจัด "ไม่มี" ใน 1.0.0/1.0.1 โดยผิด แก้เป็น **ผ่านบางส่วน** ตามหลักฐานโค้ดจริง (§3 — แก้ตาม codex gate p4-r1 M7) — **ไม่มีแถวใดถูกอ้างว่า "ผ่าน" โดยไม่มีหลักฐาน** · แถวบางส่วนทั้ง 7: AUTH-009 · AUTH-011 · CAT-003 · LRN-009 · ASM-011 · CRB-008 · AUD-005
...
| LRN-009 resume ตำแหน่งล่าสุด | **ผ่านบางส่วน** (แก้ตาม gate p4-r1 M7 — ฉบับ 1.0.0/1.0.1 จัด "ไม่มี" ผิด) | การ์ด "เรียนต่อจากที่ค้าง" จากหน้ารวม = 1 คลิก (continue-card.tsx:2,5 — AC ≤ 2 คลิก) · เครื่องเล่นวิดีโอ seek ไปตำแหน่งเริ่มดูต่อเองเมื่อโหลดเสร็จ (video-player.tsx:59-62) · ตำแหน่งเริ่มดูต่อคำนวณจาก watch % (learning.server.ts:355-359) โดย progress route บันทึก `video_max_position_sec` จริง · **ข้อจำกัด**: ตำแหน่งที่เดินต่อเป็นค่าประมาณจากสัดส่วนการรับชม ไม่ใช่วินาทีเป๊ะ ๆ ที่หยุดค้างไว้ — จึงเป็น "บางส่วน" |
...
หมายเหตุ: รายการของตารางนี้ (1.1.0 เหลือเพียง ASM-012(S)) เป็น **ความจริงของ as-built v1 ที่ RTM วางแผนไว้กว้างกว่า** — บันทึกเปิด ไม่ปิดบัง และไม่นับเข้า coverage ที่อ้างผ่านของ §3 · **แก้ไขตาม codex gate p4-r1 M7**: LRN-009 เคยอยู่ในตารางนี้ (1.0.0/1.0.1 จัด "ไม่มีฟิลด์/endpoint ตำแหน่งสื่อ") — การตรวจซ้ำพบว่า resume มีจริงในโค้ด (การ์ดเรียนต่อ + ตัวเล่น seek อัตโนมัติจาก watch % — หลักฐานที่ §3) จึงย้ายออกจากหนี้ไปเป็น **ผ่านบางส่วน** · สิ่งที่ยังเป็นข้อจำกัดจริงของ LRN-009 คือความละเอียดของตำแหน่ง (ประมาณจาก % ไม่ใช่วินาทีเป๊ะ) — ถ้าต้องการปิดถึงระดับนั้น ให้ใช้ `video_max_position_sec` ที่บันทึกอยู่แล้วมา seed ตัวเล่นแทน (งานอนาคต ไม่ใช่หนี้ "ไม่มี")

---

## 6. คำวินิจฉัยชุดทดสอบ (สรุปปิด Phase 2)

- ชุดทดสอบที่รันจริงครอบครบทุกฟังก์ชันหลักที่ส่งมอบใน v1 (81/83 FR มีหลักฐาน = ผ่าน 74 + ผ่านบางส่วน 7 ตาม census ของ §3 · อีก 1 ปิดตามนโยบาย AUTH-003 · ไม่มีใน as-built เหลือ 1 รายการ ASM-012(S) — AUTH-004/005/010 ปิดโดย Wave G P1 [#92] (1.1.0) · LRN-009 แก้เป็นผ่านบางส่วนตาม gate p4-r1 M7 — ตัวเลขปรับตาม gate p4-r2 M5) และ NFR กลุ่ม security ทั้งหมดมีหลักฐานระดับโค้ด+ชุดทดสอบ
- ความเสี่ยงที่เหลืออยู่รูปธรรม: (1) กลุ่ม PERF รอวัดจริงบน prod (2) การโจมตีเชิงมนุษย์/business logic รอ external pentest (3) หนี้ 1 รายการของ §5.3 (ASM-012(S))
- ตัวเลข Wave G P1 ผูกกับ develop `fc790c8` (post-merge battery: unit 2445/2445 · tsc 0 · lint 0 · build 0 · integration 194/194 · health 200 · e2e 38 passed + 1 flaky ผ่าน retry RC=0 · รอบแรกหลัง merge IT เจอ fetch failed 4 เคสจากช่วง recompile ของ dev server หลังสลับ branch — รันซ้ำผ่าน 194/194 · หลักฐาน `/tmp/ltc-it-pm{,2}.log`) · ตัวเลขฉบับ 1.0.x เดิมผูกกับ `96f99c0` — rerun ได้ตามสูตร §2.4 โดยไม่ต้องตีความเพิ่ม

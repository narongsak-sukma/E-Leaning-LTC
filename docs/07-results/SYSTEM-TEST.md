...
| ASM-012 ทบทวนข้อสอบหลังสอบ (S) | **ผ่านบางส่วน** (1.2.0 แก้วินิจฉัย — เดิม 1.0.0–1.1.0 จัด "ไม่มี" เพราะ grep ตาม path ของ RTM `/attempts/{id}/review` ซึ่งไม่มีจริง) | หน้าทบทวนจริง **`/my/exams/{attemptId}`** (`src/app/(learner)/my/exams/[attemptId]/page.tsx:154-210` — โจทย์ · ตัวเลือก ถูก/ที่เลือก · คำอธิบาย · คะแนนต่อข้อ · ข้อความ "ยังไม่เปิดเฉลยตามกติกา" เมื่อ view ปิด) · `GET /attempts/{id}/result` unit **×17** (ส่งตาม view ไม่ filter/เปิดเอง · row-drift fail-closed · 404 ไม่เปิดเผยเจ้าของ · aal2 · rate) · `learner_attempt_view` 0009:22-77 บังคับ baseline after_final_attempt (นับเฉพาะครั้งที่ส่งแล้ว — D20-B1) · e2e-08 หน้าผล (ผ่าน + คะแนนตรง DB) · **ช่องว่างที่เหลือ**: ยังไม่เปิดเฉลยเมื่อ "ผ่านแล้ว" (ครึ่งหลังของ AC — ผ่านแล้วยังเริ่มสอบใหม่ได้ 0048:118-122 นับเฉพาะจำนวนครั้ง จึงยังต้องปิดกั้นกันรั่วเฉลย) · คอลัมน์ `exam_review_mode` config ต่อหลักสูตร (S) ยังไม่มีในสคีมา (0009:20 จดไว้) |
...
**ปิดแล้วโดยการตรวจซ้ำ 1.2.0 (Wave G P2 `[#92]`)**: ASM-012(S) — เดิม 1.0.0–1.1.0 จดว่า "ไม่มี `/attempts/{id}/review`" จากการ grep ตาม path ของ RTM ซึ่งเป็น path ที่ไม่มีจริงใน as-built — การตรวจซ้ำพบว่าหน้าทบทวนข้อสอบมีอยู่จริงที่ `/my/exams/{attemptId}` ผ่าน `GET /attempts/{id}/result` ใต้ `learner_attempt_view` (หลักฐานเต็มที่แถว §3) จึงย้ายออกจากหนี้ไปเป็น **ผ่านบางส่วน** · สิ่งที่ยังเป็นข้อจำกัดจริง (ส่งต่อ Wave G P3): ยังไม่เปิดเฉลยเมื่อ "ผ่านแล้ว" ตามครึ่งหลังของ AC + คอลัมน์ config ต่อหลักสูตร `exam_review_mode` ยังไม่มีในสคีมา

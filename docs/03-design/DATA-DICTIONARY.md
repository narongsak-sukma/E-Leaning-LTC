...
| `mfa_backup_codes_replace` / `_consume` / `_status` / `_invalidate` (0042) | replace: atomic ลบ+ใส่ 8-12 hash + audit `AUTH_MFA_BACKUPS_REGENERATED` · consume: single-use UPDATE … WHERE used_at IS NULL + audit `AUTH_MFA_BACKUP_CODE_USED` คืน {valid, remaining} · status: metadata ล้วน {generated, unused, total, lastGeneratedAt} (ไม่มีโค้ด/hash ในผลลัพธ์) · invalidate: ลบทั้งชุด + audit `AUTH_MFA_BACKUPS_INVALIDATED` | user-JWT (auth.uid) ทั้งชุด · replace บังคับมี factor TOTP verified (D12-10 ห้ามเหลือ 0 factor ใช้ได้จริง) + hash 8-12 ค่า hex64 distinct |

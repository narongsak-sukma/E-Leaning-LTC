...
| claimed_at | timestamptz | NULL (lease ของ claim — `claim_data_export_job` ตั้ง ณ ยึดงาน · เกิน 10 นาที = claim ถัดไปยึดคืนได้ ไม่ติด processing ตลอดกาล — gate p5-r1 B7) |
...
| `claim_data_export_job` / `complete_data_export_job` / `fail_data_export_job` | worker: claim (pending→processing, คืน job_id ทีละแถว) / complete (→done + file_media_id + audit `DATA_EXPORT_DONE` + event `data_export.ready`) / fail (→failed + error) | service_role เท่านั้น (job token ของ internal route) |

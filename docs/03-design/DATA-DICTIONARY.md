...
| cursor_submitted_at | timestamptz | NULL (0028 MAJOR-1 — ตำแหน่งเดินคิวล่าสุด commit พร้อม counts ต่อใบ; null = ยังไม่เริ่ม/จบ sweep) |
...
Worker/procedure (0028 — canonical; แทน step/tick ของ 0027 ตาม codex gate r2 MAJOR-1/2 · picker v2/ตาราง/cron/grants ยังเป็นของ 0027):
...
- **`cert_auto_issue_tick(p_course_id, p_max_certs, inout p_result)`** — รูปแบบเดินคิวเดียวกัน (commit ต่อใบ · เดินผ่านแถวล้ม) แต่**รับขอบเขตหลักสูตร** (`p_course_id null` = ทุกหลักสูตร — cron ใช้ค่านี้; test/ops ส่งหลักสูตรเจาะจง — M3: ไม่แตะแถวนอกขอบเขต) · actor ระบบ `00000000-0000-4000-8000-00000000a170` mode='auto' · flag `cert_auto_issue` ปิด → `{skipped:true,reason:'flag_off'}` (ไม่ audit) · advisory lock **ต่อ scope** (0028 — `'ltc:cert-auto-issue:'||scope_key` · tick ของ test กับของ cron คนละ scope ไม่ตีกัน; แถวทับกันกันซ้ำด้วย uq ของใบอยู่แล้ว) กำลังรัน → `{skipped:true,reason:'already_running'}` · **cursor ต่อ scope ในตาราง `cert_auto_cursor` (0028 MAJOR-1)**: commit พร้อมใบต่อใบ = tick ถัดไปเดินต่อข้าม CALL จริง · เมื่อ sweep จบ (คิวหมด — ชนเพดานก่อนจะ probe ยืนยันก่อนตัดสินใจ) → **reset cursor เป็น null = รอบ retry ใหม่**: แถวที่เคยล้มถูกลองใหม่รอบถัดไป (bounded ต่อ tick ด้วย p_max_certs · แถวใหม่ submitted_at ใหม่กว่าอยู่หัวคิวอยู่แล้ว — AC ≤5 นาทีคงเดิม)
...
#### `cert_auto_cursor` — cursor ของ auto tick ต่อ scope **(DCR-8 · 0028 · codex gate r2 MAJOR-1)**

วัตถุประสงค์ (ตารางใหม่ 0028): จุดเดินคิวล่าสุดของ `cert_auto_issue_tick` แยกตามขอบเขต — `scope_key` = uuid หลักสูตร (tick แบบระบุหลักสูตร) หรือ `'*'` (tick ของ cron ทุกหลักสูตร) — commit พร้อมใบ+audit เป็น TX เดียวต่อใบ ทำให้ tick รอบถัดไปเดินต่อ**ข้าม CALL** จริง (at-least-once — ไม่มีแถวถูกข้าม) · sweep จบ (คิวหมด) = reset เป็น null → รอบ retry ใหม่ของแถวที่เคยล้ม

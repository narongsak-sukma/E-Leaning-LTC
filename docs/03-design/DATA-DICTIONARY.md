...
|          |                                                   |
| -------- | ------------------------------------------------- |
| เวอร์ชัน | 1.4.0 — Wave G P3 (feat/wave-g-p3 · [#93] · แผน D81 ผ่าน D81/D88 · gate 7 รอบ r1→r7 สุดท้าย **PASS 0B/0M/0m** → merge `db988dd`): migrations **0047-0052** — 0047 (Wave G P2 D75): RPC `admin_set_question_status` (เปิด/ปิดข้อสอบทางเดียว — transition matrix + version+1 + audit TX เดียว) · 0048 (Wave G P2 D77): `start_attempt` pool cardinality re-check หลัง selection (two-statement Read Committed race — probe IT จับจริง) · **0049 (Wave G P3 D83-D87)**: enum `exam_review_mode` ('after_final_attempt'/'never') + คอลัมน์บน `assessment_rules` + recreate `learner/instructor_attempt_view` (lateral current rules + NOT EXISTS in_progress + passed-OR — invariant คู่พลัง D84: เฉลยเปิด ⟺ ไม่มี attempt ค้าง ∧ mode ≠ 'never' ∧ (ครบ max ตาม D20-B1 ∨ passed)) + `start_attempt` B3.5 (หลัง max_attempts ก่อน cooldown → **ERR-ASM-007** 422 — ผู้ผ่านบล็อกสอบซ้ำ) + RPC `admin_add_assessment_rules` (version = max+1 ใน INSERT…SELECT เดียว + unique_violation retry ×3) + trigger **ASSESSMENT_CONFIG_CHANGE** ครอบ 3 ทางเขียน (RPC ใหม่ · POST v1 เดิม · insert ตรง ar_write) · 0050: column grant `selection` ให้ authenticated (prefill โมดัลกติกา GP3-r1 M2) · 0051: ปิดทางอ่านตรง PostgREST ของผู้เรียน + RPC `admin_latest_assessment_rules` ฝั่งเจ้าหน้าที่ (GP3-r2 R2-M3) · 0052: audience ของ RPC mirror RLS ar_read 0010 (GP3-r3 R3-M2) — §3.4 แถวคอลัมน์ใหม่ + §3.1 บล็อกฟังก์ชัน +3 แถว · ไม่มีตารางใหม่ (นับตารางคง 49)<br>1.3.7 — codex gate r1 fix wave (feat/wave-f · [#91] · migration **0045**): §3.1 ตารางใหม่ **`mfa_backup_attempts`** (lockout consume ผิด 5 ครั้ง/ล็อก 15 นาที — F4) และ **`mfa_pending_stash`** (token ขั้น password เข้ารหัส AES-256-GCM เก็บ server-side · cookie เก็บ uuid · peek/consume split — F2/F5) · RPC: mfa_backup_codes_replace/invalidate +guard **aal2 ที่ตัว RPC** + advisory lock ต่อผู้ใช้ (F1/F6) · consume คืน `locked` + นับ fail · 3 RPC ใหม่ mfa_pending_stash_create/take/consume (take/consume EXECUTE anon — ขั้นสองยังไม่มี session · uuid = bearer secret 122 บิต อายุ 300 วิที่ DB) · trigger 0044 → **fail-closed** (F7 — audit ล้ม = update อีเมลล้ม) · consents_from_signup dedupe คีย์ซ้ำใน array เดียว (F10) · นับตาราง 47→**49** · 1.3.6 — DCR Wave F F-0 (lead · 0040/0042/0044 · `[#91]`): §3.1 ตารางใหม่ **`mfa_backup_codes`** (sha256-only · 1 ชุด 8-12 โค้ด/ผู้ใช้ · single-use ผ่าน used_at · ไม่มี policy ผู้ใช้เลย — ทางเดียวคือ 4 RPC 0042 · policy `app_owner_*` ต่อ action + `ltc_app_owner_read_mfa` บน auth.mfa_factors — บทเรียน: column grant อย่างเดียวบนตาราง auth.* ที่เปิด RLS = SECURITY DEFINER เห็น 0 แถวเงียบ ๆ) + บล็อกฟังก์ชัน §3.1 เพิ่ม 3 แถว ครอบ 6 ชื่อฟังก์ชัน (mfa_backup_codes_replace/consume/status/invalidate · my_audit_email_change_request · purge_expired_retention ใต้ purge_role) + §4.6 ตาราง retention ตรงกับ 0040 ครบ (7 ตาราง v1 · audit_logs/learning-records = v1.1) · นับตาราง 44→**47** (จับค่าเดิมค้างหลัง 3 ตาราง — course_categories/cert_auto_cursor/cert_auto_walked — pipe-audit F-6) · 1.3.5 — codex gate p5-r3 fix wave (feat/wave-e-phase5 · [#90]): 0039 §1 `add column if not exists claim_token/claimed_at` ปิดช่อง upgrade จาก baseline (gate r3 BLOCKER: baseline `0e69e00` apply 0036 ฉบับก่อนแก้ → runner ข้าม 0036 ตาม ledger → คอลัมน์ต้องเกิดที่ 0039 เองก่อน backfill — พิสูจน์จริงตามที่ gate สั่ง: DB ใหม่ apply baseline 36 ไฟล์ (claimed_at/claim_token = **0** คอลัมน์) → `make migrate` → 0038+0039 ผ่าน runner ล้วน → คอลัมน์ครบ ledger 38 → dcr11+dcr12 **13/13 บน DB ที่ upgrade จาก baseline นั้น** ครบสาย worker จริง → เมล → ผู้รับกดลิงก์ 200 — ไม่มีการแก้ schema ด้วยมือ) · **storage-api v1 ของ image ปัจจุบันถอดคอลัมน์ `storage.buckets.public` ออก** — insert bucket ของ 0019/0035/0036 เลือกตาม schema ที่เจอ (มีคอลัมน์ = ใส่ `false` เหมือนเดิม · ไม่มี = สองคอลัมน์ — ทุก bucket เป็น private ตามนโยบายเดิมอยู่แล้ว เข้าถึงผ่าน signed URL/นโยบายเท่านั้น; latency ของบั๊ก: volume เดิมเกิดตอน image ยังมีคอลัมน์ จึงไม่มีใครเจอจนสร้าง baseline DB จริงรอบนี้) · seed.sql ตั้ง GUC **สองตัว** (`request.jwt.claims` + `request.jwt.claim.sub` — auth.uid() ของ image 15.8.1 อ่านแบบแยกฟิลด์ของ PostgREST; ตั้ง claims เฉย ๆ → auth.uid()=NULL → my_roles() ว่าง → guard_question_activation ปฏิเสธ → seed ทั้ง TX กลิ้ง — runtime ผ่าน gateway ไม่เจอเพราะ Kong/PostgREST ตั้งครบทั้งคู่) · 1.3.4 — codex gate p5-r2 fix wave (feat/wave-e-phase5 · [#90] · migration **0039** forward-fixes — พิสูจน์จริง DCR-12 integration 7/7 รวม worker จริงครบสายถึงผู้รับกดลิงก์): §3.1 `data_export_jobs` += **`claim_token uuid`** (fence ของรอบการถือครอง — `claim` ออก `gen_random_uuid()` ทุกรอบ · `complete`/`fail` ต้องแนบ token ที่ตรงผู้ถือปัจจุบัน: ไม่ตรง = RAISE `(ERR-VAL-001\|stale_lease)` errcode **22023** — worker เก่าที่ lease ถูกยึดปิดงานคนอื่นไม่ได้ · gate p5-r2 M1) + `claimed_at` ขยายความหมาย reclaim สองทาง: เกิน 10 นาที **หรือ IS NULL** (แถว processing ก่อนมี lease ของ B4 ถูก claim ถัดไปยึดกลับ — backfill 0039 §1) + **สัญญา path สองชั้นของไฟล์ export (B1)**: คีย์ที่ส่ง `storage.from('pdpa-exports').upload()` = `{user_id}/{job_id}.json` "ในบักเก็ต" เท่านั้น (storage-js เติม bucketId อีกชั้น — ส่ง path เต็มจะได้ object `pdpa-exports/pdpa-exports/…` คนละที่กับที่ email worker ลงนาม) ขณะที่ `media_assets.storage_path` เก็บ **path เต็ม** `pdpa-exports/{user_id}/{job_id}.json` · **origin ลิงก์ในเมล์ = มุมมองผู้รับ (r2 hostname ruling)**: email worker เขียนทับ protocol+host ของ signed URL ด้วย env `SUPABASE_PUBLIC_URL` ก่อนใส่เมล์ (dev = `http://localhost:8000` — container เห็น `kong:8000` ผู้รับเปิดไม่ได้ · ไม่ตั้ง = ใช้ URL จาก `SUPABASE_URL` ตรง ๆ → prod โดเมนเดียวไม่ต้องตั้ง · ค่าผิดรูปแบบ = fail แถว `download_url_invalid`) · RPC worker เปลี่ยนลายเซ็น (0039 §2): `claim_data_export_job()` คืน `{jobId, userId, claimToken}` · `complete_data_export_job(p_job_id, p_file_media_id, p_chunks, p_request_id, p_claim_token)` · `fail_data_export_job(p_job_id, p_error, p_claim_token)` · 0039 §3 ย้ำนิยาม final idempotent ของ 6 ฟังก์ชันที่ fix เคยแก้ใน migration เก่า (my_request_account_deletion/confirm_account_deletion/admin_grant_role/admin_revoke_role/notification_dispatch_tick/email_complete — pattern forward-migration: runner ข้ามไฟล์ที่ ledger มีแล้วโดยชื่อ แก้ในไฟล์เดิมไม่มีผลกับ DB ที่เคย apply · gate p5-r2 BLOCKER-3) · 1.3.3 — codex gate p5-r1 (feat/wave-e-phase5 · [#90]): §3.5 `data_export_jobs` เพิ่มคอลัมน์ `claimed_at` (lease ของ claim — worker ตายเกิน 10 นาที claim ถัดไปยึดงานคืน · B7) + อีเมล `data_export.ready` ตัวแปรเทมเพลตเป็นตัวระบุเท่านั้น (`full_name`/`job_id`/`file_media_id` — signed URL 604800s ผู้กลาง email worker ลงนามจาก `media_assets` ตอนส่ง · B3) · §3.x `email_outbox.payload` ลิงก์ bearer (`confirm_url`/`download_url`) ถูกแทน `[redacted-after-send]` อะตอมกับเปลี่ยนสถานะ sent (M1 · 0034 — แถว fail คงลิงก์ไว้ให้ retry) · `account_deletion_requests` RPC ขอ/ยืนยันคืน `userId` ของผู้ขอ (B1) · 1.3.2 — ปรับให้ตรง as-built ของ BFF license (lane B): §3.1 `lawyer_licenses` หมายเหตุ conflict ของ `admin_decide_license_application` เปลี่ยนรหัสตาย `ERR-PRF-001` → **`ERR-VAL-001\|license_no_conflict`** (RPC emit จริง · BFF แปลง 409 · พิสูจน์ DCR-11 เคส e — พร้อมตัด ERR-PRF-001/002 ออกจากทะเบียน API-SPEC และ `src/lib/errors.ts`) · 1.3.1 — 0036 r2 (Wave E Phase 5 · D-p5-14 follow-up): §3.6 `email_outbox` กำกับ **carve-out ธุรกรรมบังคับ NTF-005** — consent gate ของ worker ยกเว้น 5 template บังคับ (`license.application.approved/rejected` · `data_export.ready` · `account.delete.confirm`/`account.deleted`) ผ่านตัวจำแนก `notification_email_mandatory` (0036 §12) ที่ประตูทั้งสองจุด (tick INSERT + worker claim) · 1.3.0 — DCR-11 (Wave E Phase 5 · D-p5-14): migrations **0035/0036** — §3.1 ตารางใหม่ **`data_export_jobs`** (PDPA export job — worker ประกอบ JSON → บักเก็ตส่วนตัว `pdpa-exports` → แจ้งผ่าน signed URL อายุ 7 วัน) และ **`account_deletion_requests`** (token hash sha256 เท่านั้น · single-use 24 ชม.) + บล็อกฟังก์ชันกลไกท้าย §3.1: `my_submit_license_application` (คำขอ+audit `LICENSE_BIND` atomic · ตรวจ evidence เจ้าของ) · `admin_decide_license_application` (TX เดียว: ตัดสิน+INSERT lawyer_licenses verified+มอบ role lawyer idempotent+audit `LICENSE_VERIFY`/`ROLE_GRANT`+event `license.application.*`) · `admin_grant_role`/`admin_revoke_role` (super_admin ห้ามผ่าน endpoint · บัญชีต้องมี role ≥1 · registrar จำกัด lawyer) · `admin_list_users` (keyset) · `admin_decide_course` (publish/unpublish/return — audit คู่ mutation TX เดียว) · `my_request_data_export`/`claim`/`complete`/`fail_data_export_job` · `my_request_account_deletion`/`confirm_account_deletion` (GoTrue ban + `profiles.deleted_at` + display_name → "บัญชีที่ขอลบแล้ว" + audit `PROFILE_DELETE` atomic) · `admin_dashboard_stats` · `admin_list_audit_logs` — ทุก RPC admin ตรวจ roles + **aal2 ในตัว** (แบบแผน 0032 B3) · §3.1 `profiles`: กำกับ **first/last_name = ข้อมูลนิติบุคคล แก้ผ่านเจ้าหน้าที่เท่านั้น** (self-edit เขต guard 0010 = display_name/phone/preferred_locale — D20-M2) + `deleted_at` มีคนเขียนจริง = RPC `confirm_account_deletion` · §3.6 notification_templates seed เพิ่ม 6 คีย์ (0035): `license.application.approved/rejected` · `data_export.ready` · `account.delete.confirm`/`account.deleted` (ธุรกรรมบังคับ NTF-005) · บักเก็ตสตอเรจใหม่ `license-evidence` + `pdpa-exports` (ส่วนตัวทั้งคู่) · นับตาราง 41→**43** · 1.2.0 — DCR-9 (Wave E Phase 3 · D68): Credit Bank ทำจริง (migration 0031) — §3.5 เพิ่มบล็อกฟังก์ชันกลไก: `credit_cycle_defaults()` (จุด canonical เดียวของ default Q1 — D8) · `ensure_renewal_cycle()` (รอบ lazy: anchor = ใบอนุญาตอนุมัติล่าสุด → role lawyer เก่าสุด · สร้างเฉพาะรอบปัจจุบัน ไม่ backfill) · `credit_accrual_tick()` = consumer ของ event `credit.accrual` (pg_cron `ltc-credit-accrual` ทุก 1 นาที · commit ต่อ event · idempotent ผ่าน partial UNIQUE · citizen = processed + `no_cycle_target` ไม่เขียน ledger) · `admin_credit_adjust()` · `my_credit_summary()`/`my_credit_transcript()` · reversal-on-revoke ใน TX เพิกถอน (idempotent ต่อ original_entry_id — CRT-006) · `submit_attempt_core` v3 เพิ่ม `required_credits_per_cycle` ลง event snapshot (event เก่าที่ค้างคิว → tick ใช้ default แทน) · §3.7 event_outbox กำกับคู่ producer/consumer แรกของ topic `credit.accrual` · 1.1.7 — แก้ความหมายคอลัมน์ sweep_top_* ตาม 0030 (หัวของรอบ ไม่ใช่ขอบบน/ผู้มาใหม่) + กำกับข้ออ้าง "ช่วงต่อเนื่อง" ของ 1.1.5 ว่าถูกหักล้าง (codex gate r5 MINOR-3) · 1.1.6 — DCR-8 walked-markers (migration 0030 ตาม codex gate r4 MAJOR-1/MAJOR-2 + MINOR-1): สถานะ "เดินแล้ว" ของ sweep แยกออกจาก watermark เป็น **marker ต่อแถว** ในตารางใหม่ `cert_auto_walked` (§3.7 — scope_key, sweep_epoch, attempt_id) + picker ใหม่ `admin_cert_auto_unmarked_pick` (เรียง desc — ใหม่สุดก่อน) + `cert_auto_issue_tick` เดิน unmarked ทีละแถว: แถวที่ commit ช้า (submitted_at = now() ตอน*เริ่ม* TX — visibility สลับลำดับกับเวลาได้) ไม่ตกหล่นอีก · `cert_auto_cursor` += `sweep_epoch` + คู่ cursor/sweep_top เหลือเป็น**ข้อมูลชี้แจง** (ไม่ใช่เงื่อนไขความถูกต้อง) · สถานะขัดที่ 0028 ค้างไว้ (คู่เดียว null) = normalize เป็น sweep ใหม่ · 1.1.5 — DCR-8 arrival-first tick (migration 0029 ตาม codex gate r3 MAJOR-1 + MINOR-1): `cert_auto_cursor` เพิ่มคอลัมน์ `sweep_top_submitted_at`/`sweep_top_attempt_id` (ขอบบนของ sweep) + picker ใหม่ `admin_cert_auto_fresh_pick` + `cert_auto_issue_tick` เป็น **tick สองเฟส** — fresh lane รับ "ผู้มาใหม่" เหนือขอบบน*ก่อน* backlog: ผู้ผ่านเงื่อนไขที่มากลาง sweep ได้ใบใน tick ถัดไปทันที ไม่รอ sweep เก่าจบ (AC ≤5 นาที — 0028 ทำให้เขาล่องหนถึงนาที 8) · ชุดที่เดินแล้วของ sweep = ช่วงต่อเนื่อง [cursor, sweep_top] โดยก่อสร้าง (ข้ออ้างนี้ถูก gate r4 หักล้าง — visibility สลับลำดับกับ submitted_at ได้ ดู 1.1.6) · 1.1.4 — DCR-8 cursor durable (migration 0028 ตาม codex gate r2 MAJOR-1/2 + MINOR-3): `cert_bulk_jobs` เพิ่มคอลัมน์ `cursor_submitted_at`/`cursor_attempt_id` + ตารางใหม่ `cert_auto_cursor` (§3.7) — step/tick เดินคิวต่อจาก cursor ที่ commit ไว้**ข้าม CALL** (at-least-once ไม่มีแถวถูกข้าม) · เพดาน 100k ของ job ตรวจก่อนลูปและก่อน branch ชนเพดาน call · builtin ทั้งหมด qualify `pg_catalog` · 1.1.3 — DCR-8 worker redesign (migration 0027 ตาม codex gate r1 M1/M2/M3): เปลี่ยน runner/tick เป็น **procedure commit ต่อใบ** `admin_cert_bulk_issue_step`/`cert_auto_issue_tick` + picker v2 มี cursor เดินผ่านแถวที่ล้ม + BFF POST insert-only 202 (worker pg_cron ทุกนาที) + ข้อจำกัด PG ≥15 (procedure ที่ COMMIT ห้าม SECURITY DEFINER/SET search_path) · 1.1.2 — DCR-8 (Wave E Phase 2, migration 0026): ตารางใหม่ `feature_flags` (§3.7) + RPC runner `admin_cert_bulk_issue_run`/`cert_auto_issue_tick` (CRT-008) + `cert_issue_core` เพิ่ม `p_mode` manual/bulk/auto ลง audit context + policy `app_owner` ของ `cert_bulk_jobs` ให้ runner (D55-5/D55-7) — *runner/tick ของ 0026 ถูก 0027 แทนแล้ว* · 1.1.1 — DCR-7 (Wave E): view `course_exam_summary` เพิ่มคอลัมน์ `assessment_id` (PB-17) · `question_snapshot` เพิ่มคีย์ `type` (PB-18) · ตารางใหม่ `cert_bulk_jobs` (§3.7) · อ่านบทเรียนยอม enrollment active/completed (ปิด D39 — D55-4) · 1.1.0 — additive (DCR-4): enum `course_level` + `courses.level`/`outcome_highlights` + views `course_public_stats`/`course_instructors_public`/`course_exam_summary` · 1.0.0 ผ่าน CTO gate (codex รอบ 5: PASS — D17) · แก้ตาม D8–D16 · baseline สำหรับ Wave B |
| วันที่    | 2026-09-13                                        |
...
| first_name / last_name | text | NULL ได้ (เก็บน้อยที่สุด — ใช้เมื่อจำเป็นพิมพ์ใบประกาศ) — **ข้อมูลนิติบุคคล (DCR-11): แก้ผ่านเจ้าหน้าที่เท่านั้น** (registrar/super_admin ผ่าน admin endpoint + audit `USER_UPDATE`) — เจ้าของแก้เองไม่ได้ (guard 0010 เขต self-edit = display_name/phone/preferred_locale/pdpa_consented_at — D20-M2); ผู้ใช้ใช้ `display_name` เป็นชื่อแสดงของตัวเอง |
...
| deleted_at | timestamptz | NULL (soft delete — เขียนจริงโดย RPC `confirm_account_deletion` 0036 พร้อม GoTrue ban + display_name → "บัญชีที่ขอลบแล้ว" ใน TX เดียวกับ audit `PROFILE_DELETE`; อ่านได้แต่ไม่มี path UPDATE อื่น) |
...
RLS: **SELECT** เจ้าของแถว หรือ staff:registrar/super_admin; **INSERT** service_role ผ่าน BFF + trigger `consents_from_signup` (0043 · SECURITY DEFINER app_owner — จุดเกิดแถว source='register' ตามด้านล่าง); **UPDATE/DELETE** ไม่อนุญาต (หลักฐาน consent แก้ไม่ได้ — ถอน = เพิ่มแถว action='revoke')
จุดเกิดแถว (Wave F 0043 · D-f-3): `source='register'` เกิดจาก **trigger `consents_from_signup` บน auth.users** (AFTER UPDATE OF email_confirmed_at · transition null→non-null เท่านั้น — กันบัญชีอีเมลตายสร้างแถว) อ่าน `raw_user_meta_data->'consents_granted'` จาก signUp (checkbox สมัคร — แตะเฉพาะที่ติ๊ก) กรองเฉพาะ key `marketing`/`email_notify` + เวอร์ชันผ่าน regex + dedupe กับแถว grant เดิม · trigger ล้ม = RAISE WARNING **ไม่บล็อกการยืนยันอีเมล** (การยืนยันตัวตนสำคัญกว่าแถว consent — ตรวจทาง PII_ACCESS/seed ได้) · เวอร์ชันมาจาก `SIGNUP_CONSENT_POLICY_VERSION` ของ config (BFF แนบตอน signUp)
...
#### `data_export_jobs` — งานส่งออกข้อมูลของเจ้าของ (PDPA data portability — IDENT-008/SEC-011 · 0036)

วัตถุประสงค์ (DCR-11): job ส่งออกข้อมูลผู้ใช้คนเดียวต่อแถว — สร้างโดย `my_request_data_export` (BFF `GET /profile/export` ตอบ 202 ทันที) แล้ว worker (internal job route ตามแบบแผน email-dispatch — loop 30 วิ) claim ทีละแถว ประกอบ JSON ทุกก้อน (โปรไฟล์/บทบาท/consent/ลงทะเบียน/ความคืบหน้า/ผลสอบ/ใบ/credit/การแจ้งเตือน) อัปโหลดบักเก็ตส่วนตัว `pdpa-exports` — **คีย์ที่ส่ง `.upload()` คือ `{user_id}/{job_id}.json` "ในบักเก็ต" เท่านั้น** (storage-js เติม bucketId เองอีกชั้น ส่ง path เต็มจะได้ object ซ้อนสอง prefix คนละที่กับที่ email worker ลงนาม — gate p5-r2 B1) ขณะที่ `media_assets.storage_path` เก็บ **path เต็ม** `pdpa-exports/{user_id}/{job_id}.json` → event `data_export.ready` → แจ้งเตือน in-app+อีเมล — ตัวแปรเทมเพลตเป็น **ตัวระบุเท่านั้น** (`full_name`,`job_id`,`file_media_id`) แล้ว **email worker ลงนาม signed URL อายุ 7 วัน (604800s) จาก `media_assets` ตอนส่ง** (URL ไม่เก็บใน outbox/ไม่ log — gate p5-r1 B3)

| คอลัมน์ | ชนิด | Constraints / Default |
| ------- | ---- | --------------------- |
| user_id | uuid | NOT NULL FK→profiles |
| status | text | NOT NULL DEFAULT 'pending', CHECK IN ('pending','processing','done','failed') |
| file_media_id | uuid | NULL FK→media_assets (แถว media ของไฟล์ JSON — มีเมื่อ done) |
| requested_at | timestamptz | NOT NULL DEFAULT now() |
| completed_at | timestamptz | NULL (มีเมื่อ done/failed) |
| error | text | NULL (ข้อความล้ม — ไม่ใส่ PII) |
| claimed_at | timestamptz | NULL (lease ของ claim — `claim_data_export_job` ตั้ง ณ ยึดงาน · reclaim สองทาง: เกิน 10 นาที **หรือ IS NULL** (แถว processing ที่ค้างก่อนมี lease ถูก claim ถัดไปยึดกลับ — backfill 0039 §1 · gate p5-r2 B4) ไม่ติด processing ตลอดกาล — B7 · คอลัมน์ทั้งสองเกิดแบบ `add column if not exists` **ที่ 0039 เอง** — ครอบทุกสถานะ DB: ใหม่ (0036 ฉบับแก้สร้างให้แล้ว = no-op) · baseline-upgrade (0039 เพิ่มให้ — gate p5-r3 BLOCKER) · apply 0039 ไปแล้ว (skip ตาม ledger และมีคอลัมน์โดยสภาพ TX-per-file)) |
| claim_token | uuid | NULL (fence รอบการถือครอง — เกิดใหม่ `gen_random_uuid()` ทุก claim · complete/fail ต้องแนบ token ที่ตรงผู้ถือปัจจุบัน ไม่ตรง = `(ERR-VAL-001\|stale_lease)` errcode 22023 — worker เก่าที่ lease ถูกยึดปิดงานไม่ได้ · gate p5-r2 M1 · 0039) |
คีย์/Index: UNIQUE(user_id) WHERE status IN ('pending','processing') (กันยื่นซ้ำขณะค้าง — BFF map เป็น 409); INDEX(status, requested_at) (worker scan); CHECK (status='done' → file_media_id IS NOT NULL)
RLS: **SELECT** เจ้าของแถวเท่านั้น (สถานะของตัวเอง); **INSERT** ผ่าน RPC `my_request_data_export` (user-JWT — ตรวจ pending ซ้ำ + cooldown 24 ชม. จาก done ล่าสุดในตัว + audit `DATA_EXPORT_REQUEST` atomic); **UPDATE** service_role เท่านั้น (worker claim/complete/fail ผ่าน RPC ต่อแถว — claim เป็น subtx เพื่อ commit สถานะ processing ก่อนประกอบไฟล์); **DELETE** ไม่อนุญาต
Retention: แถว job ตลอดอายุบัญชี + 10 ปี (หลักฐานการใช้สิทธิ์); ไฟล์ในบักเก็ต = อายุตาม signed URL 7 วันแล้วลบได้ (housekeeping Wave F)

#### `account_deletion_requests` — คำขอลบบัญชีของเจ้าของ (SEC-012 soft-delete · 0036)

วัตถุประสงค์ (DCR-11): หลักฐานการขอลบ + กลไกยืนยันตัวตนสองชั้น (อีเมล token) — สร้างโดย `my_request_account_deletion` (BFF `POST /profile/delete` — guard: ผู้ถือ instructor/staff:*/super_admin ห้ามลบเอง 403) ยืนยันผ่าน route สาธารณะ `GET /profile/delete/confirm?token=…` → RPC `confirm_account_deletion` single-use

| คอลัมน์ | ชนิด | Constraints / Default |
| ------- | ---- | --------------------- |
| user_id | uuid | NOT NULL FK→profiles |
| token_hash | text | NOT NULL (**sha256 ของ token เท่านั้น — ตัว token (CSPRNG 43 อักษร) ไม่เก็บที่ไหนในระบบ**) |
| expires_at | timestamptz | NOT NULL (requested + 24 ชม.) |
| status | text | NOT NULL DEFAULT 'pending', CHECK IN ('pending','confirmed','expired') |
| requested_at | timestamptz | NOT NULL DEFAULT now() |
| confirmed_at | timestamptz | NULL (มีเมื่อ confirmed) |
คีย์/Index: UNIQUE(token_hash); INDEX(user_id, requested_at DESC); CHECK (status='confirmed' → confirmed_at IS NOT NULL)
RLS: **SELECT** service_role เท่านั้น (ผู้ใช้ไม่อ่านตารางนี้ผ่าน client ได้เลย — token เป็น secret); **INSERT** ผ่าน RPC `my_request_account_deletion` (user-JWT — สร้าง token ส่งออกทางอีเมลเท่านั้น · audit ไม่บันทึก event แยก: การยืนยันจริงคือ event `PROFILE_DELETE`); **UPDATE** ผ่าน RPC `confirm_account_deletion` เท่านั้น (single-use: อัปเดต pending→confirmed ด้วย WHERE status='pending' AND expires_at > now() — แถวที่ใช้ไป/หมดอายุไม่ถูกแตะซ้ำ) + ใน TX เดียว: GoTrue ban (BFF ทำหลัง RPC สำเร็จ) + `profiles.deleted_at` + display_name → "บัญชีที่ขอลบแล้ว" + audit `PROFILE_DELETE` atomic; **DELETE** ไม่อนุญาต
Retention: ตลอดอายุบัญชี + 10 ปี (หลักฐานการใช้สิทธิ์ลบ — audit/ผลสอบเก็บต่อตาม §4.6)

#### `mfa_backup_codes` — โค้ดสำรอง MFA (GoTrue ไม่มีในตัว · 0042 · D11-12)

วัตถุประสงค์ (Wave F D-f-1): โค้ดสำรอง 1 ชุดต่อผู้ใช้ มี 8-12 โค้ด ใช้แทน TOTP ตอน login ขั้นสองได้ทีละโค้ด (single-use · RPC ยอมรับ 8-12 โค้ด/ชุด — REST regenerate ออก 8 เสมอ `BACKUP_CODE_COUNT`) — เก็บ **sha256 hash เท่านั้น** โค้ดจริงไม่อยู่ที่ไหนในระบบ (BFF สุ่ม CSPRNG รูป `xxxx-xxxx` · แสดงครั้งเดียวตอนสร้าง · สัญญา hash single-source: `encode(sha256(convert_to(lower(btrim(code)),'UTF8')),'hex')` = `crypto.createHash('sha256').update(code.trim().toLowerCase()).digest('hex')` ฝั่ง Node (normalize ที่สองฝั่งก่อนแฮชเสมอ) — พิสูจน์ตรงกันแล้ว 8ee88ff0…)

| คอลัมน์ | ชนิด | Constraints / Default |
| ------- | ---- | --------------------- |
| id | uuid | PK DEFAULT gen_random_uuid() |
| user_id | uuid | NOT NULL FK→profiles ON DELETE CASCADE |
| code_hash | text | NOT NULL CHECK `^[0-9a-f]{64}$` |
| used_at | timestamptz | NULL (= ยังไม่ใช้ · single-use บังคับโดย UPDATE … WHERE used_at IS NULL) |
| created_at | timestamptz | NOT NULL DEFAULT now() |
คีย์/Index: UNIQUE(user_id, code_hash) (lookup ของ consume + กัน hash ซ้ำต่อผู้ใช้)
RLS: **ไม่มี policy ของ anon/authenticated เลย** — ผู้ใช้เจ้าของก็เข้าถึงตรง ๆ ไม่ได้ ทางเดียวคือ 4 RPC (SECURITY DEFINER owner `app_owner` · policy `app_owner_select/insert/update/delete_mfa_backup_codes` ต่อ action ตามแบบแผนบ้าน — grant อย่างเดียวโดน RLS บล็อก) + `grant select (user_id, factor_type, status) on auth.mfa_factors to app_owner` คู่ policy `ltc_app_owner_read_mfa` (ตาราง auth.* เปิด RLS เจ้าของ supabase_auth_admin — column grant อย่างเดียว SECURITY DEFINER เห็น 0 แถวเงียบ ๆ)
Retention: ตามรอบของบัญชี — invalidate ทั้งชุดเมื่อ unenroll factor TOTP สุดท้าย/ปิด MFA (RPC `mfa_backup_codes_invalidate`)

#### `mfa_backup_attempts` — ตัวนับ/ล็อกการใช้โค้ดสำรองผิด (0045 · gate r1 F4)

วัตถุประสงค์ (Wave F F-0 fix): จำกัด brute-force ผ่าน RPC `mfa_backup_codes_consume` — ผิด 5 ครั้ง/ผู้ใช้ = ล็อก 15 นาที (locked_until ตรวจ**ก่อน**แมตช์โค้ด — โค้ดถูกก็ถูกกันขณะล็อก และไม่ถูกเผา) · สำเร็จ = ลบแถว (รีเซ็ตนับ) · fail_count รีเซ็ตเป็น 1 เมื่อพ้นหน้าต่างล็อก

| คอลัมน์ | ชนิด | Constraints / Default |
| ------- | ---- | --------------------- |
| user_id | uuid | PK FK→profiles ON DELETE CASCADE |
| fail_count | int | NOT NULL DEFAULT 0 CHECK ≥ 0 |
| last_fail_at | timestamptz | NULL |
| locked_until | timestamptz | NULL (= ไม่ล็อก) |
คีย์/Index: PK user_id (แถวเดียวต่อผู้ใช้ — upsert จาก consume)
RLS: **ไม่มี policy ของ anon/authenticated** — เข้าถึงผ่าน `mfa_backup_codes_consume` เท่านั้น (policy `app_owner_all_mfa_backup_attempts` ของ SECURITY DEFINER)
Retention: อายุสั้นตามธรรมชาติ — สำเร็จ/ผู้ใช้ถูกลบ = แถวหาย (cascade)

#### `mfa_pending_stash` — stash ชั่วคราวของ login สองขั้น (0045 · gate r1 F2/F5)

วัตถุประสงค์ (Wave F F-0 fix): เดิม cookie `ltc_mfa_pending` แพ็กคู่ token จริงเป็น JSON (ใครอ่าน Set-Cookie ได้ = ได้ session aal1 เต็มตัวโดยไม่ผ่านขั้นสอง) — ตอนนี้ token ถูกเข้ารหัส **AES-256-GCM ฝั่งแอป** (env `LTC_MFA_PENDING_KEY` base64 32 ไบต์ · prod บังคับโดย config schema · ไม่มี fallback) เก็บเป็น ciphertext ในตารางนี้ · cookie เก็บ **uuid v4 อย่างเดียว** (bearer secret 122 บิต) · อายุ 300 วิ + single-use บังคับที่ DB ไม่ใช่ browser maxAge

| คอลัมน์ | ชนิด | Constraints / Default |
| ------- | ---- | --------------------- |
| id | uuid | PK DEFAULT gen_random_uuid() (= ค่าใน cookie) |
| user_id | uuid | NOT NULL FK→profiles ON DELETE CASCADE |
| payload | text | NOT NULL — `v1.<iv>.<tag>.<ct>` base64url (≤8192) · DB เห็น ciphertext เท่านั้น |
| expires_at | timestamptz | NOT NULL (now()+300s ตอนสร้าง — RPC ทั้งสองเช็ค `expires_at > now()`) |
| consumed_at | timestamptz | NULL (= ยังใช้ได้ · consume เซ็ตค่า = single-use จริง) |
| created_at | timestamptz | NOT NULL DEFAULT now() |
คีย์/Index: INDEX(expires_at) · หนึ่ง pending ต่อผู้ใช้ — `mfa_pending_stash_create` ลบแถวเดิมของ user ก่อน insert
RLS: **ไม่มี policy ของ anon/authenticated** — ผ่าน 3 RPC เท่านั้น (policy `app_owner_all_mfa_pending_stash`)
Retention: เก็บกวาดในตัว RPC (create/take): หมดอายุ/ใช้แล้วเกิน 1 ชั่วโมง = ลบ (ไม่ต้องมี cron ของตารางนี้)

**peek/consume split**: `take` = peek (อ่าน payload ไม่หมกเหตุ — พิมพ์รหัสผิดพยายามใหม่ได้เท่าอายุที่เหลือ ตามแบบแผน UX dcr14 เคส c) · `consume` = RPC แยกที่ BFF เรียก**หลัง verify สำเร็จเท่านั้น** → uuid ตายจริงพร้อมการออก session (replay คุกกี้เดิม = expired)

#### ฟังก์ชันกลไก Identity/License/PDPA/Admin (0035 + 0036 + 0040/0042/0044 — DCR-11 + Wave F F-0)

แบบแผนเดียวกับบล็อก §3.5/0032: **mutation+audit atomic ใน TX เดียว** (§1.5 AUDIT-DESIGN — audit ล้ม = rollback ทั้งรายการ) · RPC ฝั่ง admin ตรวจ **roles + aal2 ในตัว** (ปิดช่อง JWT aal1 เรียก PostgREST ตรง — 0032 B3) · เรียกจาก BFF ด้วย user JWT:

| Function | ทำอะไร (TX เดียวกันทั้งแถว) | เงื่อนไขที่ตรวจข้างใน |
| -------- | --------------------------- | --------------------- |
| `my_submit_license_application` | INSERT license_applications(pending) + audit `LICENSE_BIND` (license_hash เท่านั้น) | auth.uid() = เจ้าของ · evidence_media_id เป็นแถว media ที่ uploaded_by = auth.uid() · ไม่มี pending ค้าง (uq → map 409) |
| `admin_decide_license_application` | UPDATE คำขอ (lock pending) → approve: ตรวจ uq license_no คนอื่น (เจอ = RAISE ชื่อ constraint) + INSERT lawyer_licenses(verified) + resulting_license_id + INSERT role_assignments lawyer (NOT EXISTS idempotent) + audit `LICENSE_VERIFY`+`ROLE_GRANT` + event `license.application.approved\|rejected` / reject: reason ≥10 + status/rejected_reason | roles (license:verify) + aal2 · คำขอต้อง pending |
| `admin_grant_role` / `admin_revoke_role` | INSERT/UPDATE role_assignments + audit `ROLE_GRANT`/`ROLE_REVOKE` | roles (role:grant/revoke) + aal2 · ชุด role ที่ยอม: lawyer (super_admin หรือ registrar — registrar เฉพาะ lawyer), instructor/staff:* (super_admin) · **super_admin ห้าม** · revoke: บัญชีเป้าหมายต้องมี role อื่นเหลือ ≥1 |
| `admin_decide_course` | UPDATE courses.status (published/draft/returned) + audit `COURSE_PUBLISH`/`COURSE_UNPUBLISH`/`COURSE_RETURN` | roles (course:publish) + aal2 · return: comment ≥10 บังคับ |
| `admin_list_users` | SELECT ค้นหา (keyset cursor) — read-only, คนเรียกตรวจสิทธิ์ user:view · **0041 v2**: แถวขาออกเพิ่ม `is_banned`/`banned_until` (left join auth.users ผ่าน column grant + policy `ltc_app_owner_read_ban` — ตาราง auth.* เปิด RLS) | roles + audit `PII_ACCESS` ผูกที่ BFF (fail-closed แบบแผน 1.1.2 B7) |
| `my_request_data_export` | INSERT data_export_jobs(pending) + audit `DATA_EXPORT_REQUEST` | auth.uid() · ไม่มี pending/processing ค้าง · cooldown 24 ชม. จาก done ล่าสุด |
| `claim_data_export_job` / `complete_data_export_job` / `fail_data_export_job` | worker: claim (pending→processing ทีละแถว — reclaim เมื่อ lease เกิน 10 นาที/IS NULL · คืน `{jobId, userId, claimToken}`) / complete (→done + file_media_id + audit `DATA_EXPORT_DONE` + event `data_export.ready` — ลายเซ็น 0039: `p_job_id, p_file_media_id, p_chunks, p_request_id, p_claim_token`) / fail (→failed + error — `p_job_id, p_error, p_claim_token`) | service_role เท่านั้น (job token ของ internal route) · **fencing: complete/fail ต้องถือ claimToken ของรอบปัจจุบัน — token เก่า = `(ERR-VAL-001\|stale_lease)` errcode 22023 (แถวคง processing)** · 0039 |
| `my_request_account_deletion` | INSERT account_deletion_requests + สร้าง token (คืนทาง function → BFF ส่งอีเมลเท่านั้น) | auth.uid() · ผู้ถือ instructor/staff:*/super_admin = RAISE 403 (SoD) |
| `confirm_account_deletion` | UPDATE request (pending→confirmed single-use) + profiles.deleted_at + display_name ซ่อน + audit `PROFILE_DELETE` | ไม่ต้อง session — ตรวจ sha256(token) + expires_at + ยัง pending (route สาธารณะเรียก) |
| `admin_dashboard_stats` | SELECT aggregate สด (ผู้ใช้ใหม่/รวม · ลงทะเบียน · ผลสอบ · credit ออก — กรองช่วงวัน) | roles (report:view) + aal2 — read-only |
| `admin_list_audit_logs` | SELECT audit_logs (filter action/actor/entity/ช่วงเวลา + keyset cursor) | roles (audit_log:view) + aal2 — read-only · audit `AUDIT_READ` ผูกที่ BFF |
| `mfa_backup_codes_replace` / `_consume` / `_status` / `_invalidate` (0042 · **0045 แก้**) | replace: atomic ลบ+ใส่ 8-12 hash + audit `AUTH_MFA_BACKUPS_REGENERATED` · consume: single-use UPDATE … WHERE used_at IS NULL + audit `AUTH_MFA_BACKUP_CODE_USED` คืน {valid, remaining, **locked**} · status: metadata ล้วน {generated, unused, total, lastGeneratedAt} (ไม่มีโค้ด/hash ในผลลัพธ์) · invalidate: ลบทั้งชุด + audit `AUTH_MFA_BACKUPS_INVALIDATED` | user-JWT (auth.uid) ทั้งชุด · **0045 (gate r1 F1/F4/F6): replace/invalidate +guard aal2 ที่ตัว RPC** (แบบแผน 0044 — ปิดช่อง JWT aal1 ยิง PostgREST ตรง; consume ห้ามใส่เพราะเป็นเส้นทาง login ที่ยัง aal1) · replace บังคับมี factor TOTP verified (D12-10) + hash 8-12 ค่า hex64 distinct + **pg_advisory_xact_lock ต่อผู้ใช้** (กัน DELETE→INSERT คร่อมกัน — F6) · consume + **advisory lock + lockout ผิด 5 ครั้ง/ล็อก 15 นาที** (ตาราง `mfa_backup_attempts` — F4) · รูปร่างโค้ดเพี้ยนไม่นับ fail |
| `mfa_pending_stash_create` / `_take` / `_consume` (0045 · gate r1 F2/F5) | create: เก็บ ciphertext AES-256-GCM (แอปเข้ารหัสก่อนส่ง) อายุ 300 วิ คืน uuid ใส่ cookie · take: **peek** คืน payload เมื่อยังไม่ใช้+ไม่หมดอายุ · consume: เซ็ต consumed_at (single-use จริง — เรียกหลัง verify สำเร็จเท่านั้น) idempotent | create: user-JWT (auth.uid) EXECUTE authenticated · take/consume: EXECUTE anon+authenticated (ขั้นสองยังไม่มี session — uuid คือ bearer secret 122 บิต อายุ/single-use บังคับที่ DB) · สอง RPC เก็บกวาดแถวหมดอายุ/ใช้แล้ว >1 ชม. เอง |
| `my_audit_email_change_request` (0044) | audit `USER_EMAIL_CHANGE_REQUEST` หลัง BFF เรียก GoTrue updateUser({email}) สำเร็จ (context = new_email_sha256 ล้วน) | user-JWT · p_new_email_sha256 ต้อง hex64 · ผู้ถือ instructor/staff:*/super_admin ต้อง aal2 (SoD เดียวกับการลบบัญชี) — trigger `audit_email_change_confirmed` บน auth.users เขียน `USER_EMAIL_CHANGE_CONFIRMED` เมื่อ email เปลี่ยนจริง (double opt-in ผ่านแล้ว) |
| `purge_expired_retention` (0040) | ลบแถวตาม §4.6 (dry-run default คืนจำนวนก่อนลบ · รันจริงเขียน audit `PURGE_EXECUTED` จำนวนต่อตาราง) — cron `ltc-purge-retention` 04:23 | SECURITY DEFINER **owner `purge_role`** · EXECUTE เฉพาะ purge_role + supabase_admin · predicate เป็น literal นิ่งในฟังก์ชัน (ไม่รับจากผู้เรียก) |
...
| uploaded_by | uuid | NOT NULL FK→profiles (ผู้อัปโหลดจริง = actor — ไม่เสมอเจ้าของข้อมูล เช่น PDF ใบประกาศ = ผู้ออกใบ) |
| deleted_at | timestamptz | NULL |
...
| exam_review_mode | exam_review_mode | NOT NULL DEFAULT 'after_final_attempt' — enum ใหม่ 0049 (D83): 'after_final_attempt' \| 'never' — คู่ invariant D84 กับ start_attempt B3.5: เฉลยเปิด ⟺ ไม่มี attempt in_progress ∧ mode ≠ 'never' ∧ (submitted-count ≥ max ตาม D20-B1 ∨ passed) |
...
#### ฟังก์ชันกลไก Credit Bank (0031 — DCR-9 · D68)

> Producer มีมาตั้งแต่ 0020: `submit_attempt_core` เมื่อตรวจผ่าน INSERT event topic **`credit.accrual`** (snapshot กฎ — F15/F17/D11-17) ใน grading TX เดียวกัน · 0031 เติมครบกลไก: consumer + รอบ lazy + RPC อ่าน/ปรับ + reversal-on-revoke — ผูกกับ SRS CRB-001..007 และ API-SPEC 1.1.0

| Function | EXECUTE (ผู้เรียก) | เขียนตาราง | พฤติกรรมสำคัญ |
| -------- | ------------------ | ----------- | -------------- |
| `credit_cycle_defaults()` | app_owner, service_role (ภายในระบบ) | — (pure) | จุด canonical **เดียว** ของ default Q1 (Appendix A/D8): รอบยาว 1 ปี · เกณฑ์ `{general: 12}` · ไม่ยกยอดข้ามรอบ — ห้าม duplicate ค่านี้ไปที่อื่น (รอยืนยัน Q1 แล้วแก้ที่จุดเดียว) |
| `ensure_renewal_cycle(p_user_id, p_on_date)` | เรียกภายในจาก tick/summary (ไม่เปิดให้ client) | renewal_cycles (INSERT) | สร้างรอบแบบ **lazy**: เป้าหมาย = ผู้ถือ role `lawyer` (หรือมีรอบอยู่แล้ว) · anchor = `license_applications` ที่อนุมัติล่าสุด (`decided_at`) → ไม่มีจึงใช้ `role_assignments` lawyer เก่าสุด · ความยาว/เกณฑ์จาก `credit_cycle_defaults()` (หรือ `required_credits_per_cycle` จาก rule snapshot ของ event ถ้ามี) snapshot ลง `required_credits` ตอนสร้าง · สร้างเฉพาะรอบปัจจุบันที่ cover `p_on_date` (**ไม่ backfill** รอบย้อนหลัง) · รอบที่ปิดแล้วห้ามแก้ · EXCLUDE (0006) บังคับไม่ให้รอบซ้อน |
| `credit_accrual_tick()` | pg_cron jobname `ltc-credit-accrual` ทุก 1 นาที + service_role (tests/ops) | credit_ledger_entries (INSERT), renewal_cycles (ผ่าน ensure), event_outbox (UPDATE status) | consumer ของ topic `credit.accrual`: advisory-xact-lock กัน tick ซ้อน · เดิน ≤5 รอบ ×200 event · **commit ต่อ event** (per-event begin/exception — event หนึ่งล้มไม่ทำแถวอื่นตาย) · ใช้ **snapshot ใน event เท่านั้น ไม่ lookup กฎซ้ำ** (retire กลางคิวไม่กระทบ — F17) · INSERT `entry_type='accrual'` idempotent ด้วย partial UNIQUE(source_type, source_id, credit_type) · ผู้ไม่มีรอบเป้าหมาย (citizen — C-4) = mark processed + note `no_cycle_target` ไม่เขียน ledger · ล้ม = attempts+1 + backoff ที่ `available_at` · attempts ≥5 → `status='failed'` · audit `CREDIT_ACCRUAL` (actor ระบบ) ณ การ INSERT ledger สำเร็จ — AC ≤5 นาที (CRB-003) |
| `admin_credit_adjust(p_user_id, p_cycle_id, p_credit_type, p_amount, p_reason, p_request_id)` | authenticated ผ่าน BFF (ตรวจสิทธิ์ staff:registrar/super_admin ในตัว) | credit_ledger_entries (INSERT) | adjustment มือ (CRB-007): `p_amount ≠ 0` signed numeric(6,2) · reason 10–500 บังคับ (ERR-CRD-002) · INSERT `entry_type='adjustment'` + `created_by = auth.uid()` + audit `CREDIT_ADJUST` (CRITICAL) ใน TX เดียว · **ไม่ใช้ service_role ใน BFF** (D68 C-9) · แจ้งเจ้าของ = Phase 4 (NTF) |
| `my_credit_summary()` | authenticated (owner-check `user_id = auth.uid()`) | renewal_cycles (lazy ผ่าน ensure) | ยอดรอบปัจจุบัน ได้รับ/ต้องมี/ขาด แยก credit_type + ประวัติรายรอบ — คำนวณจาก ledger จริง (SUM(amount)) ไม่ใช่ denorm (CRB-005) |
| `my_credit_transcript()` | authenticated (owner-check เดียวกัน) | — (อ่านอย่างเดียว) | แถว transcript รายวิชา/ผลสอบ/credit/ใบประกาศฯ (ledger+cycles+certificates) คืนเป็น json ให้ BFF เรนเดอร์ JSON หรือ CSV (UTF-8 BOM) หรือ PDF (pdf-lib ฝั่ง BFF) — CRB-006 · citizen ไม่มีแถว ledger แต่ยังเห็นหลักสูตร/ใบประกาศฯ ของตัวเอง |

**Reversal-on-revoke (CRT-006 — D68 C-5):** `admin_revoke_certificate` (v2 ใน 0031) ใน **TX เดียวกับการเพิกถอน** INSERT `entry_type='reversal'` ต่อรายการ accrual ของ attempt ต้นทาง (แยกตาม credit_type) ที่**ยังไม่ถูก reverse**: `amount = −(accrual ต้นทาง)` · `original_entry_id` ชี้แถวต้นทาง · `source_type='certificate_revocation'` · `source_id` = certificate id · reason อัตโนมัติอ้าง cert_no · `created_by` = ผู้เพิกถอน · idempotent ด้วย guard NOT EXISTS ต่อคู่ (cert, original_entry) — partial UNIQUE ของ 0006 คลุมเฉพาะ accrual จึงต้องคุมเอง · audit `CREDIT_REVERSAL` (CRITICAL) หนึ่ง event ต่อการเพิกถอน สรุป context {certificate_id, reversed_rows, total_amount} · re-issue (CRT-007) **ไม่กระทบ credit** (คงเดิม — D12-14/15)

**Snapshot ของ event `credit.accrual`:** 0020 เขียน `{source_type, source_id (attempt), user_id, enrollment_id, passed_at, rule:{rule_id, code, credits, credit_type, renewal_cycle, valid_days, carry_over}}` — 0031 (`submit_attempt_core` v3) เพิ่ม `required_credits_per_cycle` ของกฎลง snapshot ของ event ใหม่ · event เก่าที่ค้างคิว (ไม่มีฟิลด์นี้) → tick ใช้ `credit_cycle_defaults()` แทน
...
วัตถุประสงค์ (ตารางเสริมที่เพิ่มนอกรายการขั้นต่ำ): แยกการส่งอีเมลออกจาก request path + retry ได้ (SDS §8) — dev ใช้ Mailpit จับ, prod ใช้ Resend/SMTP; **worker เช็ค consent ที่ยัง active ของผู้รับ ณ ตอน dispatch** (ถอน consent = งดส่ง — F18/D12) — **ยกเว้นธุรกรรมบังคับ NTF-005 ที่ส่งเสมอ** (`license.application.approved/rejected`, `data_export.ready`, `account.delete.confirm`, `account.deleted`): ตัวจำแนก `notification_email_mandatory(template_key)` (0036 §12) ยกเว้นชุดนี้ที่ประตูอีเมลทั้งสองจุด — ตอน tick INSERT คิว (`notification_dispatch_tick`) และตอน worker claim (`email_claim_batch`) — เพราะอีเมลเหล่านี้คือสิทธิ PDPA ของเจ้าของข้อมูลเอง (ผลตัดสินใบอนุญาต/ลิงก์ส่งออกข้อมูล/การยืนยันลบบัญชี) การงดส่งเพียงเพราะผู้ใช้ยังไม่เคย grant `email_notify` ทำให้สิทธินั้นใช้ไม่ได้จริง
...
| payload | jsonb | NOT NULL DEFAULT '{}' (ตัวแปรเทมเพลต — ห้ามใส่ PII เกินจำเป็น · ลิงก์ bearer `confirm_url`/`download_url` ถูกแทน `[redacted-after-send]` อะตอมกับเปลี่ยนสถานะ sent — แถว fail คงลิงก์ไว้ให้ retry · gate p5-r1 M1/0034) |
...
> 0031 (DCR-9): คู่ producer/consumer แรกที่มีจริงของตารางนี้ = topic **`credit.accrual`** — producer `submit_attempt_core` (0020 · grading TX) · consumer `credit_accrual_tick` (0031 · pg_cron ทุก 1 นาที) — รายละเอียดพฤติกรรมที่ §3.5 บล็อกฟังก์ชันกลไก
...
| `consents_from_signup()` (0043 · Wave F) | auth.users (AFTER UPDATE OF email_confirmed_at · null→non-null) | แทรกแถว `consents` จาก `raw_user_meta_data->'consents_granted'` ตอน signUp — fail-open RAISE WARNING |
| `audit_email_change_confirmed()` (0044 · Wave F) | auth.users (AFTER UPDATE OF email) | audit `USER_EMAIL_CHANGE_CONFIRMED` เมื่ออีเมลเปลี่ยนจริง (context sha256 ล้วน) |
...
| certificate_verifications, email_outbox | 90 วัน | email_outbox แถว sent ถูกถอดลิงก์ bearer ตอนเปลี่ยนสถานะ (gate p5-r1 M1) |
...
| `admin_add_assessment_rules()` | assessment_rules (INSERT) | guards แบบแผน 0047 + version = max+1 ใน INSERT…SELECT เดียว + unique_violation retry ×3 + audit ASSESSMENT_CONFIG_CHANGE ใน TX เดียว (0049 D87 — ทางเขียน version ใหม่ของโมดัลกติกา) |
| `admin_set_question_status()` | questions (UPDATE) | เปิด/ปิดข้อสอบทางเดียว — transition matrix (draft→active→retired→active) + version+1 + audit TX เดียว (0047 D75) |
| `admin_latest_assessment_rules()` | assessment_rules (SELECT) | อ่านกติกาล่าสุด + selection ฝั่งเจ้าหน้าที่ — audience mirror RLS ar_read 0010 L692-L702 (0051+0052 — ผู้เรียนอ่าน selection ทางตรงไม่ได้) |

...
Worker/procedure (0029 — canonical ของ tick · step ของ 0028 ยัง canonical ต่อ · tick แทนของ 0028 ตาม codex gate r3 MAJOR-1 · picker v2/ตาราง/cron/grants ยังเป็นของ 0027):
...
- **`admin_cert_auto_fresh_pick(p_course_id, p_limit, p_top_submitted_at, p_top_attempt_id)`** (0029 r3 MAJOR-1 — SECURITY DEFINER `app_owner` · language sql stable) — คิว "ผู้มาใหม่" เหนือขอบบนของ sweep: eligibility เดียวกับ picker v2 แต่กลับทิศ — เรียง `(submitted_at, attempt_id)` **asc** (ผู้รอนานสุดก่อนในกลุ่มผู้มาใหม่) + เงื่อนไข `> (p_top_*)` · p_top คู่ใดเป็น null (sweep ยังไม่เริ่ม) = ไม่คืนแถว — sweep ใหม่เริ่มที่ backlog lane อยู่แล้ว · EXECUTE: `app_owner`/`postgres` เท่านั้น (revoke รวม service_role)
...
- **`cert_auto_issue_tick(p_course_id, p_max_certs, inout p_result)`** — **tick สองเฟส (0029 r3 MAJOR-1 · แทน tick ของ 0028)**: งบ v_max เดียวกันทั้งสองเฟส — **fresh lane ก่อน**: แถว "ผู้มาใหม่" = `(submitted_at, attempt_id)` ใหม่กว่าขอบบนของ sweep (`sweep_top_*` ใน cert_auto_cursor) ผ่าน `admin_cert_auto_fresh_pick` — ผู้ผ่านเงื่อนไขที่มากลาง sweep ได้ใบใน tick ถัดไปทันทีแม้ sweep เดิมยังถูกเพดานกั้น ไม่ต้องรอ sweep เก่าจบและ reset (SRS AC ≤5 นาที — 0028 ทำให้เขาล่องหนจนกว่าคิวเก่าจะหมด) · แล้ว **backlog lane** เดิน desc ใต้ cursor เหมือน 0028 (retry แถวเคยล้ม · คิวเดิม · แถวแรกที่เดินของ sweep ใหม่ = ตั้งขอบบน) · **ชุดที่เดินแล้วของ sweep = ช่วงต่อเนื่อง [cursor, sweep_top] โดยก่อสร้าง — ไม่มีแถวถูกข้าม** · รับขอบเขตหลักสูตร (`p_course_id null` = ทุกหลักสูตร — cron; test/ops ส่งหลักสูตรเจาะจง — M3 ไม่แตะแถวนอกขอบเขต) · actor ระบบ `a170` mode='auto' · flag `cert_auto_issue` ปิด → `{skipped:true,reason:'flag_off'}` (ไม่ audit) · advisory lock **ต่อ scope** (`'ltc:cert-auto-issue:'||scope_key`) กำลังรัน → `{skipped:true,reason:'already_running'}` · cursor คู่ + ขอบบน commit พร้อมใบ+audit เป็น TX เดียวต่อใบใน `cert_auto_cursor` (ข้าม CALL จริง) · sweep จบ (คิวหมด — ชนเพดานก่อนจะ probe `admin_cert_bulk_pick` ยืนยันก่อนตัดสินใจ) → **reset ทั้งสองคู่**เป็น null = รอบ retry ใหม่
...
#### `cert_auto_cursor` — cursor ของ auto tick ต่อ scope **(DCR-8 · 0028+0029 · codex gate r2 MAJOR-1/r3 MAJOR-1)**

วัตถุประสงค์ (ตารางใหม่ 0028 · ขยาย 0029): จุดเดินคิวล่าสุดของ `cert_auto_issue_tick` แยกตามขอบเขต — `scope_key` = uuid หลักสูตร (tick แบบระบุหลักสูตร) หรือ `'*'` (tick ของ cron ทุกหลักสูตร) — commit พร้อมใบ+audit เป็น TX เดียวต่อใบ ทำให้ tick รอบถัดไปเดินต่อ**ข้าม CALL** จริง (at-least-once — ไม่มีแถวถูกข้าม) · **คู่ cursor (backlog) + คู่ sweep_top (ขอบบนของ sweep — 0029)**: แถวที่ใหม่กว่าขอบบน = "ผู้มาใหม่" ที่ fresh lane รับ*ก่อน* backlog ใน tick ถัดไป (AC ≤5 นาที) · sweep จบ (คิวหมด) = reset ทั้งสองคู่เป็น null → รอบ retry ใหม่ของแถวที่เคยล้ม
...
| sweep_top_submitted_at | timestamptz | NULL (0029 r3 MAJOR-1 — ขอบบนของ sweep = แถวใหม่ที่สุดที่ถูกเดินแล้วรอบนี้; แถวที่ใหม่กว่า = "ผู้มาใหม่" ของ fresh lane · null = sweep ยังไม่เริ่ม/จบแล้ว) |

/**
 * /login — หน้าเข้าสู่ระบบ (Thai-first · Server Component — Wave C-0)
 *
 * - ยืนยันตัวตนด้วย server action (actions.ts) — session อยู่ cookie httpOnly (SDS §5.1)
 * - error ที่แสดง = เฉพาะ code จากทะเบียน src/lib/errors.ts (API-SPEC §2) —
 *   ไม่แสดงข้อความดิบจาก upstream เด็ดขาด
 * - กัน open redirect: hidden input `next` ผ่าน resolveSafeNextPath เสมอ
 */
import { errorDefinition, type ErrorCode } from "@/lib/errors";
import { resolveSafeNextPath } from "@/lib/auth/session";
import { loginAction } from "../actions";

interface LoginPageProps {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
}

/** allowlist ของ error code ที่หน้า login ยอมแสดง (ไม่รับ code อื่นจาก query string) */
const LOGIN_ERROR_CODES = ["ERR-VAL-001", "ERR-AUTH-002", "ERR-RATE-001", "ERR-SYS-001"] as const satisfies readonly ErrorCode[];

const LOGIN_NOTICES = ["registered", "email_not_confirmed"] as const;
type LoginNoticeParam = (typeof LOGIN_NOTICES)[number];

/** ข้อความไทยของ notice (fixed copy — ไม่รับจาก query string) */
const LOGIN_NOTICE_MESSAGES: Record<LoginNoticeParam, string> = {
  registered: "สมัครสมาชิกสำเร็จ กรุณาตรวจสอบอีเมลเพื่อยืนยันตัวตนก่อนเข้าสู่ระบบ",
  email_not_confirmed: "กรุณายืนยันอีเมลก่อน (เปิดลิงก์ในอีเมลที่ได้รับแล้วลองเข้าสู่ระบบอีกครั้ง)",
};

function firstParam(value: string | string[] | undefined): string | null {
  if (typeof value !== "string" || value.length === 0) {
    return null;
  }
  return value;
}

function isLoginNotice(value: string): value is LoginNoticeParam {
  return (LOGIN_NOTICES as readonly string[]).includes(value);
}

/** ตรวจ + narrow ให้เป็น ErrorCode จริง (กัน string ปลอมจาก query string เข้า errorDefinition) */
function isLoginErrorCode(value: string): value is (typeof LOGIN_ERROR_CODES)[number] {
  return (LOGIN_ERROR_CODES as readonly string[]).includes(value);
}

const inputClass =
  "w-full rounded-[10px] border-[1.5px] border-mist-300 bg-white px-3.5 py-2.5 text-ink-900 placeholder:text-ink-400 focus:border-brand-600 focus:outline-none";

const labelClass = "mb-1.5 block font-heading text-sm font-semibold text-ink-900";

export default async function LoginPage({ searchParams }: LoginPageProps) {
  const params = await searchParams;
  const next = resolveSafeNextPath(firstParam(params.next));
  const rawError = firstParam(params.error);
  const errorCode = rawError !== null && isLoginErrorCode(rawError) ? rawError : null;
  const errorDef = errorCode !== null ? errorDefinition(errorCode) : null;
  const rawNotice = firstParam(params.notice);
  const notice = rawNotice !== null && isLoginNotice(rawNotice) ? rawNotice : null;

  return (
    <main className="flex min-h-dvh items-center justify-center bg-mist-50 px-4 py-10">
      <div className="w-full max-w-md rounded-[14px] bg-white p-6 shadow-card sm:p-8">
        <p className="text-center text-sm text-ink-500">
          ระบบฝึกอบรมออนไลน์ สภาทนายความแห่งประเทศไทย
        </p>
        <h1 className="mt-1 text-center font-heading text-2xl font-bold text-brand-900">
          เข้าสู่ระบบ
        </h1>
        {errorDef !== null ? (
          <p
            id="login-alert"
            role="alert"
            className="mt-5 rounded-[10px] border-[1.5px] border-danger-200 bg-danger-50 px-4 py-3 text-sm text-danger-700"
          >
            {errorDef.message}
          </p>
        ) : null}
        {notice !== null ? (
          <p
            id="login-notice"
            role="status"
            className="mt-5 rounded-[10px] border-[1.5px] border-success-200 bg-success-50 px-4 py-3 text-sm text-success-700"
          >
            {LOGIN_NOTICE_MESSAGES[notice]}
          </p>
        ) : null}
        <form action={loginAction} className="mt-6 space-y-4">
          <input type="hidden" name="next" value={next} />
          <div>
            <label htmlFor="email" className={labelClass}>
              อีเมล <span className="text-danger-600" aria-hidden="true">*</span>
              <span className="sr-only">(จำเป็น)</span>
            </label>
            <input
              id="email"
              name="email"
              type="email"
              required
              autoComplete="email"
              aria-describedby={errorDef !== null || notice !== null ? "login-alert login-notice" : undefined}
              placeholder="name@example.com"
              className={inputClass}
            />
          </div>
          <div>
            <label htmlFor="password" className={labelClass}>
              รหัสผ่าน <span className="text-danger-600" aria-hidden="true">*</span>
              <span className="sr-only">(จำเป็น)</span>
            </label>
            <input
              id="password"
              name="password"
              type="password"
              required
              autoComplete="current-password"
              placeholder="••••••••"
              className={inputClass}
            />
          </div>
          <button
            type="submit"
            className="w-full rounded-[10px] bg-brand-600 px-[18px] py-2.5 font-heading font-semibold text-white shadow-card hover:bg-brand-700"
          >
            เข้าสู่ระบบ
          </button>
        </form>
        <p className="mt-6 text-center text-sm text-ink-600">
          ยังไม่มีบัญชีผู้ใช้{" "}
          <a
            href={`/register?next=${encodeURIComponent(next)}`}
            className="font-semibold text-brand-700 hover:underline"
          >
            สมัครสมาชิก
          </a>
        </p>
      </div>
    </main>
  );
}

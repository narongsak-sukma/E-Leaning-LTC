/**
 * route.test — GET /api/v1/admin/reports/assessments (Wave E · D55-6)
 * ตรวจ: RBAC 2 ชั้น (se/sv/sa) · query strict · x-ltc-truncated · drift → 503
 */
...
function getUrl(query = "") {
  return new Request("http://localhost:3000/api/v1/admin/reports/assessments" + query, {
    headers: { "x-forwarded-for": "10.5.0.1", "x-request-id": "req-e5-2" },
  });
}

const ROW = { assessmentId: "a1000000-0000-4000-8000-000000000001", attemptTotal: 8, attemptPassed: 6, passRatePct: 75 };
...
describe("RBAC ของ assessments", () => {
  it.each([[[SE]], [[SV]], [[SA]]])("บทบาท %j → 200", async (roles) => {
    mockAuth(roles);
    vi.mocked(listAssessmentStatistics).mockResolvedValue({ rows: [ROW], truncated: false } as never);
  ...
  it.each([[[SR]]])("บทบาท %j → 403 ERR-RBAC-001 + ไม่เรียก lib", async (roles) => {
    ...
    expect(listAssessmentStatistics).not.toHaveBeenCalled();
  });
});

describe("query + response ของ assessments", () => {
  ...
  it("truncated → header x-ltc-truncated: true · ไม่ truncated → ไม่มี header", async () => {
    mockAuth([SE]);
    vi.mocked(listAssessmentStatistics).mockResolvedValue({ rows: [], truncated: true } as never);
    const res = await GET(getUrl());
  ...
  it("แถว drift (คีย์แปลกปลอม) → 503 ERR-SYS-002 assessment_statistics_drift", async () => {
    mockAuth([SE]);
    ...
    expect(body.error.details["reason"]).toBe("assessment_statistics_drift");
  });
});

...
export const PASSWORD_CHANGE_MESSAGES = {
  ...
  // gate r1 MINOR-1: audit_failed เกิดหลัง PUT สำเร็จเท่านั้น → รหัสผ่านเปลี่ยนแล้ว
  // เสมอ — คำแนะนำเดิม ("ลองใหม่แล้วถ้าเจอซ้ำกับปัจจุบัน = สำเร็จ") จับไม่ได้จริง:
  // same_password ตรวจก่อน re-auth เทียบกับ "input" ของรหัสปัจจุบัน ไม่ใช่รหัสจริง
  // ฝั่ง server รอบลองใหม่จะเจอ wrong_current ก่อนถึงขั้นนั้นเสมอ
  audit_failed:
    "เปลี่ยนรหัสผ่านเรียบร้อยแล้ว แต่ระบบบันทึกหลักฐานการเปลี่ยนไม่สำเร็จ โปรดเข้าสู่ระบบด้วยรหัสผ่านใหม่ต่อไป (หากเข้าสู่ระบบไม่ได้ กรุณาใช้ลิงก์ลืมรหัสผ่านเพื่อตั้งใหม่)",
